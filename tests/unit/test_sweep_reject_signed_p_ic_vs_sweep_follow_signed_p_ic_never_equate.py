"""sweep_reject_signed_p_ic ≠ sweep_follow_signed_p_ic — never-equate (H33≠H34)."""
...
def test_keys_and_h33_h34_bind() -> None:
    assert "sweep_reject_signed_p_ic" != "sweep_follow_signed_p_ic"
    assert H33_HYPOTHESIS_ID != H34_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert (
        sweep_reject_signed_p_ic_vs_sweep_follow_signed_p_ic_never_equate_honesty_errors(receipt)
        == []
    )
