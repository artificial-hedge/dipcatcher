"""gap_finite_rate ≠ H23 session_reconstructs_daily_rate — never-equate soft-verify."""
...
def test_keys_and_hyp_gates_distinct() -> None:
    assert "gap_finite_rate" != "session_reconstructs_daily_rate"
    ...
    assert H23_HYPOTHESIS_ID not in {H20_HYPOTHESIS_ID, H21_HYPOTHESIS_ID}
    ...
    assert not northset_has_finite_book_uncrossed_rate(
        {"session_reconstructs_daily_rate": 1.0, **elig}
    )
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert gap_finite_rate_vs_session_reconstructs_never_equate_honesty_errors(receipt) == []
