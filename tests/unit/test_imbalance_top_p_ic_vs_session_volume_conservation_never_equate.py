"""imbalance_top_p_ic ≠ session_volume_conservation_rate — never-equate (H22↔H24)."""
...
def test_keys_gates_and_h24_bind() -> None:
    assert "imbalance_top_p_ic" != "session_volume_conservation_rate"
    assert H22_HYPOTHESIS_ID != H24_HYPOTHESIS_ID
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key["session_volume_conservation_rate"] == H24_HYPOTHESIS_ID
    assert by_key.get("imbalance_top_p_ic") != H24_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert (
        imbalance_top_p_ic_vs_session_volume_conservation_never_equate_honesty_errors(receipt) == []
    )
