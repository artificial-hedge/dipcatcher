"""H41 sweep_reject_fold_positive_fraction ≠ gap_finite_rate — never-equate."""
...
def test_keys_and_h41_bind() -> None:
    assert "sweep_reject_fold_positive_fraction" != "gap_finite_rate"
    assert H41_HYPOTHESIS_ID not in {H20_HYPOTHESIS_ID, H21_HYPOTHESIS_ID, H22_HYPOTHESIS_ID}
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key["sweep_reject_fold_positive_fraction"] == H41_HYPOTHESIS_ID
    assert by_key.get("gap_finite_rate") != H41_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert (
        sweep_reject_fold_positive_fraction_vs_gap_finite_never_equate_honesty_errors(receipt) == []
    )
