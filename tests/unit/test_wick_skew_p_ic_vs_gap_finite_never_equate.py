"""H26 wick_skew_p_ic ≠ gap_finite_rate — never-equate soft-verify."""
...
def test_keys_and_h26_bind() -> None:
    assert "wick_skew_p_ic" != "gap_finite_rate"
    assert H26_HYPOTHESIS_ID not in {H20_HYPOTHESIS_ID, H21_HYPOTHESIS_ID, H22_HYPOTHESIS_ID}
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key["wick_skew_p_ic"] == H26_HYPOTHESIS_ID
    assert by_key.get("gap_finite_rate") != H26_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert wick_skew_p_ic_vs_gap_finite_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert wick_skew_p_ic_vs_gap_finite_never_equate_honesty_errors({"wick_skew_p_ic": 0.05}) == []
    assert wick_skew_p_ic_vs_gap_finite_never_equate_honesty_errors({"gap_finite_rate": 1.0}) == []


def test_helper_registered() -> None:
    assert (
        wick_skew_p_ic_vs_gap_finite_never_equate_honesty_errors in NORTHSET_RECEIPT_HONESTY_HELPERS
    )
