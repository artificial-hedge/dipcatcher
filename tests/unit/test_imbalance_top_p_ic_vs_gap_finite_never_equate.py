"""H22 imbalance_top_p_ic ≠ gap_finite_rate — never-equate soft-verify."""
...
def test_keys_and_hyp_gates_distinct() -> None:
    assert "imbalance_top_p_ic" != "gap_finite_rate"
    assert H22_HYPOTHESIS_ID not in {H20_HYPOTHESIS_ID, H21_HYPOTHESIS_ID}
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert imbalance_top_p_ic_vs_gap_finite_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert (
        imbalance_top_p_ic_vs_gap_finite_never_equate_honesty_errors({"imbalance_top_p_ic": 0.05})
        == []
    )
    assert (
        imbalance_top_p_ic_vs_gap_finite_never_equate_honesty_errors({"gap_finite_rate": 1.0}) == []
    )
