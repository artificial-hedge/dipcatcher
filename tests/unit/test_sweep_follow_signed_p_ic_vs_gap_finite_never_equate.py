"""H34 sweep_follow_signed_p_ic ≠ gap_finite_rate — never-equate soft-verify."""
...
def test_keys_and_h34_bind() -> None:
    assert "sweep_follow_signed_p_ic" != "gap_finite_rate"
    assert H34_HYPOTHESIS_ID not in {H20_HYPOTHESIS_ID, H21_HYPOTHESIS_ID, H22_HYPOTHESIS_ID}
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key["sweep_follow_signed_p_ic"] == H34_HYPOTHESIS_ID
    assert by_key.get("gap_finite_rate") != H34_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert sweep_follow_signed_p_ic_vs_gap_finite_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert (
        sweep_follow_signed_p_ic_vs_gap_finite_never_equate_honesty_errors(
            {"sweep_follow_signed_p_ic": 0.05}
        )
        == []
    )
    assert (
        sweep_follow_signed_p_ic_vs_gap_finite_never_equate_honesty_errors({"gap_finite_rate": 1.0})
        == []
    )
