"""Day Wave 132: named unbiased sample path for optimize_asof / trailing covariance."""
...
def _fake_sample(returns: np.ndarray) -> tuple[np.ndarray, dict[str, float | str]]:
...
def test_named_sample_path_returns_trailing_matrix_with_overlay(tmp_path: Path) -> None:
    cfg = _cfg(tmp_path)
    cfg.optimizer.covariance = "sample"
    frame = _panel()
    ...
    with (
        patch("quant_fund.pipeline.forecast.sample", side_effect=capture),
        patch(
            "quant_fund.pipeline.forecast.dcc_gaussian",
            side_effect=AssertionError("sample must not silently run dcc_gaussian"),
        ),
        patch(
...
def test_named_sample_fails_closed_on_short_history(tmp_path: Path) -> None:
    ...
    ids = frame["security_id"].unique().sort().to_list()
    hist = frame.filter(pl.col("event_time") <= asof)
    with pytest.raises(ValueError, match="optimizer_covariance_failed:sample"):
        estimate_optimizer_covariance_asof(cfg, frame, asof, ids, hist)


def test_named_sample_keeps_sample_when_t_gt_n(tmp_path: Path) -> None:
    cfg = _cfg(tmp_path)
    cfg.optimizer.covariance = "sample"
    frame = _panel(n_days=80, n_names=3)
    asof = frame["event_time"].unique().sort().to_list()[-1]
    ids = frame["security_id"].unique().sort().to_list()
    hist = frame.filter(pl.col("event_time") <= asof)
    seen: dict[str, int] = {}
    ...
    with (
        ...
        patch(
            "quant_fund.pipeline.forecast.ledoit_wolf",
            side_effect=AssertionError("sample must not switch to Ledoit-Wolf when T>N"),
        ),
        patch(
            "quant_fund.pipeline.forecast.oas",
            side_effect=AssertionError("sample must not switch to OAS when T>N"),
        ),
        patch(
            "quant_fund.pipeline.forecast.apply_market_variance_overlay_to_covariance",
...
def test_named_sample_does_not_fall_back_to_ledoit_wolf(tmp_path: Path) -> None:
    cfg = _cfg(tmp_path)
    cfg.optimizer.covariance = "sample"
    frame = _panel()
    asof = frame["event_time"].unique().sort().to_list()[-1]
    ...
    with (
        patch("quant_fund.pipeline.forecast.sample", side_effect=boom),
        patch(
            "quant_fund.pipeline.forecast.ledoit_wolf",
            side_effect=AssertionError("failed sample must not run Ledoit-Wolf"),
        ),
        patch(
            "quant_fund.pipeline.forecast.oas",
            side_effect=AssertionError("failed sample must not run OAS"),
        ),
        patch(
            "quant_fund.pipeline.forecast.ewma",
            side_effect=AssertionError("failed sample must not run ewma"),
        ),
        pytest.raises(ValueError, match="optimizer_covariance_failed:sample"),
    ):
        estimate_optimizer_covariance_asof(cfg, frame, asof, ids, hist)


def test_named_sample_fails_closed_on_family_mismatch(tmp_path: Path) -> None:
    cfg = _cfg(tmp_path)
    cfg.optimizer.covariance = "sample"
    frame = _panel()
    asof = frame["event_time"].unique().sort().to_list()[-1]
    ...
    with (
        patch("quant_fund.pipeline.forecast.sample", side_effect=lw_params),
        pytest.raises(ValueError, match="unexpected_family:ledoit_wolf"),
    ):
        estimate_optimizer_covariance_asof(cfg, frame, asof, ids, hist)


def test_optimize_asof_named_sample_stamps_trailing_identity(tmp_path: Path) -> None:
    cfg = _cfg(tmp_path)
    cfg.optimizer.covariance = "sample"
    frame = _panel()
    asof = frame["event_time"].unique().sort().to_list()[-1]
    ...
    with (
        patch("quant_fund.pipeline.forecast.sample", side_effect=_fake_sample),
        patch(
            "quant_fund.pipeline.forecast.ledoit_wolf",
            side_effect=AssertionError("optimize_asof sample must not run Ledoit-Wolf"),
        ),
    ...
    assert set(weights["covariance_estimator"].to_list()) == {OPTIMIZER_COVARIANCE_SAMPLE}
    assert set(weights["covariance_object"].to_list()) == {OPTIMIZER_COVARIANCE_OBJECT_TRAILING}
    assert set(weights["covariance_spec"].to_list()) == {SAMPLE_SPEC_UNBIASED}
