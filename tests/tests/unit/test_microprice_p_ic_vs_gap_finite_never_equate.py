"""microprice_p_ic ≠ gap_finite_rate — never-equate (H25)."""
...
def test_keys_and_h25_bind() -> None:
    assert "microprice_p_ic" != "gap_finite_rate"
    assert H25_HYPOTHESIS_ID not in {
        H20_HYPOTHESIS_ID,
        H21_HYPOTHESIS_ID,
        H22_HYPOTHESIS_ID,
    }
    ...
    assert by_key.get("gap_finite_rate") != H25_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert microprice_p_ic_vs_gap_finite_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert (
        microprice_p_ic_vs_gap_finite_never_equate_honesty_errors({"microprice_p_ic": 0.05}) == []
    )
