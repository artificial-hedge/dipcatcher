"""dm_gk_vs_park_p ≠ gap_finite_rate — never-equate (H28)."""
...
def test_keys_and_h28_bind() -> None:
    assert "dm_gk_vs_park_p" != "gap_finite_rate"
    assert H28_HYPOTHESIS_ID not in {
        H20_HYPOTHESIS_ID,
        H21_HYPOTHESIS_ID,
        H22_HYPOTHESIS_ID,
    }
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key["dm_gk_vs_park_p"] == H28_HYPOTHESIS_ID
    assert by_key.get("gap_finite_rate") != H28_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert "dm_gk_vs_park_p" in receipt
    assert "gap_finite_rate" in receipt
    assert dm_gk_vs_park_p_vs_gap_finite_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert (
        dm_gk_vs_park_p_vs_gap_finite_never_equate_honesty_errors({"dm_gk_vs_park_p": 0.05}) == []
    )
