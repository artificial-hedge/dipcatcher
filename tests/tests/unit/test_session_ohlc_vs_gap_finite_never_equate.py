"""session_ohlc_identity_rate ≠ gap_finite_rate — never-equate soft-verify."""
...
def test_keys_and_hyp_gates_distinct() -> None:
    assert "session_ohlc_identity_rate" != "gap_finite_rate"
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key.get("session_ohlc_identity_rate") != H23_HYPOTHESIS_ID
    assert by_key.get("gap_finite_rate") != H23_HYPOTHESIS_ID
    assert H20_HYPOTHESIS_ID not in {H21_HYPOTHESIS_ID, H23_HYPOTHESIS_ID}
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert session_ohlc_vs_gap_finite_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert (
        session_ohlc_vs_gap_finite_never_equate_honesty_errors({"session_ohlc_identity_rate": 1.0})
        == []
    )
    assert session_ohlc_vs_gap_finite_never_equate_honesty_errors({"gap_finite_rate": 1.0}) == []


def test_helper_registered() -> None:
    assert (
        session_ohlc_vs_gap_finite_never_equate_honesty_errors in NORTHSET_RECEIPT_HONESTY_HELPERS
    )
