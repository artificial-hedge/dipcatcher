"""mean_session_close_imbalance receipt + CLI; ≠ path / daily imbalance means."""
...
def test_stamps_mean_session_close_imbalance_when_session_l2_on() -> None:
    bars = SyntheticMarketProvider(n_assets=4, n_days=24, seed=13).get_bars()
    cfg = AppConfig()
    ...
    # Honesty: distinct receipt keys (last-snap / path); do not equate identities.
...
def test_nan_when_session_l2_off() -> None:
    ...
    receipt = bench_northset(bars, cfg)
    assert math.isnan(float(receipt["mean_session_close_imbalance"]))


def test_northset_cli_echoes_mean_session_close_imbalance() -> None:
    ...
    assert "mean_session_close_imbalance=" in result.output
