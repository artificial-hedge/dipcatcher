"""sweep_follow_event_p ≠ gap_finite_rate — never-equate (H36↔gap)."""
...
def test_keys_and_h36_bind() -> None:
    assert "sweep_follow_event_p" != "gap_finite_rate"
    assert H36_HYPOTHESIS_ID not in {
        H20_HYPOTHESIS_ID,
        H21_HYPOTHESIS_ID,
        H22_HYPOTHESIS_ID,
    }
    ...
    assert by_key.get("gap_finite_rate") != H36_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert sweep_follow_event_p_vs_gap_finite_never_equate_honesty_errors(receipt) == []
