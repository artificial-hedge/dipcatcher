"""ohlc_identity_rate ≠ gap_finite_rate — never-equate polish."""
...
def test_keys_and_h20_gate_only_on_ohlc() -> None:
    assert "ohlc_identity_rate" != "gap_finite_rate"
    assert H20_HYPOTHESIS_ID not in {H21_HYPOTHESIS_ID, H22_HYPOTHESIS_ID}
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert ohlc_identity_vs_gap_finite_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert (
        ohlc_identity_vs_gap_finite_never_equate_honesty_errors({"ohlc_identity_rate": 1.0}) == []
    )


def test_helper_registered() -> None:
    assert (
        ohlc_identity_vs_gap_finite_never_equate_honesty_errors in NORTHSET_RECEIPT_HONESTY_HELPERS
    )
