"""ofi_p_ic ≠ gap_finite_rate — never-equate (H27)."""
...
def test_keys_and_h27_bind() -> None:
    assert "ofi_p_ic" != "gap_finite_rate"
    assert H27_HYPOTHESIS_ID not in {
        H20_HYPOTHESIS_ID,
        H21_HYPOTHESIS_ID,
        H22_HYPOTHESIS_ID,
    }
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key["ofi_p_ic"] == H27_HYPOTHESIS_ID
    assert by_key.get("gap_finite_rate") != H27_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert "ofi_p_ic" in receipt
    assert "gap_finite_rate" in receipt
    assert ofi_p_ic_vs_gap_finite_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert ofi_p_ic_vs_gap_finite_never_equate_honesty_errors({"ofi_p_ic": 0.05}) == []


def test_helper_registered() -> None:
    assert ofi_p_ic_vs_gap_finite_never_equate_honesty_errors in NORTHSET_RECEIPT_HONESTY_HELPERS
