"""session_ohlc_identity_rate ≠ session_reconstructs_daily_rate — never-equate."""
...
def test_keys_and_h23_bind() -> None:
    assert "session_ohlc_identity_rate" != "session_reconstructs_daily_rate"
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert session_ohlc_vs_reconstructs_never_equate_honesty_errors(receipt) == []
...
def test_helper_registered() -> None:
    assert (
        session_ohlc_vs_reconstructs_never_equate_honesty_errors in NORTHSET_RECEIPT_HONESTY_HELPERS
    )
