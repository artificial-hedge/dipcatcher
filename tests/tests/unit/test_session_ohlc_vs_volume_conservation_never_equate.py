"""session_ohlc_identity_rate ≠ session_volume_conservation_rate — never-equate."""
...
def test_keys_and_h24_bind() -> None:
    assert "session_ohlc_identity_rate" != "session_volume_conservation_rate"
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key["session_volume_conservation_rate"] == H24_HYPOTHESIS_ID
    assert by_key.get("session_ohlc_identity_rate") != H24_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert session_ohlc_vs_volume_conservation_never_equate_honesty_errors(receipt) == []
