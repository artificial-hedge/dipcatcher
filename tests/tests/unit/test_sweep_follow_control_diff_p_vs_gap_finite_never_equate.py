"""H45 sweep_follow_control_diff_p ≠ gap_finite_rate — never-equate soft-verify."""
...
def test_keys_and_h45_bind() -> None:
    assert "sweep_follow_control_diff_p" != "gap_finite_rate"
    assert H45_HYPOTHESIS_ID not in {H20_HYPOTHESIS_ID, H21_HYPOTHESIS_ID, H22_HYPOTHESIS_ID}
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key["sweep_follow_control_diff_p"] == H45_HYPOTHESIS_ID
    assert by_key.get("gap_finite_rate") != H45_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert "sweep_follow_control_diff_p" in receipt
    assert "gap_finite_rate" in receipt
    assert sweep_follow_control_diff_p_vs_gap_finite_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert (
        sweep_follow_control_diff_p_vs_gap_finite_never_equate_honesty_errors(
            {"sweep_follow_control_diff_p": 0.05}
        )
        == []
    )
    assert (
