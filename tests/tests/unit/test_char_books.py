...
def test_lagged_book_ignores_todays_score() -> None:
    ...
    shocked = frame.with_columns(
        pl.when(
            (pl.col("security_id") == "BBB") & (pl.col("event_time") == day0 + timedelta(days=5))
        )
        .then(pl.lit(99.0))
        .otherwise(pl.col("score"))
        .alias("score")
    )
    _, base = lagged_characteristic_book(
        frame, "score", prefer_high=True, k_frac=0.4, one_way_cost=0.0
    )
    _, alt = lagged_characteristic_book(
        shocked, "score", prefer_high=True, k_frac=0.4, one_way_cost=0.0
    )
