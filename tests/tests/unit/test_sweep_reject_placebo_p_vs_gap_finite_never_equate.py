"""H37 sweep_reject_placebo_p ≠ gap_finite_rate — never-equate soft-verify."""
...
def test_keys_and_h37_bind() -> None:
    assert "sweep_reject_placebo_p" != "gap_finite_rate"
    assert H37_HYPOTHESIS_ID not in {H20_HYPOTHESIS_ID, H21_HYPOTHESIS_ID, H22_HYPOTHESIS_ID}
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key["sweep_reject_placebo_p"] == H37_HYPOTHESIS_ID
    assert by_key.get("gap_finite_rate") != H37_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert sweep_reject_placebo_p_vs_gap_finite_never_equate_honesty_errors(receipt) == []
...
def test_one_key_absent_skipped() -> None:
    ...
    assert (
        sweep_reject_placebo_p_vs_gap_finite_never_equate_honesty_errors({"gap_finite_rate": 1.0})
        == []
    )
