"""ohlc_identity_rate ≠ session_chain_rate — never-equate (H20 ≠ H29)."""
...
def test_keys_gates_and_h29_bind() -> None:
    assert "ohlc_identity_rate" != "session_chain_rate"
    assert H20_HYPOTHESIS_ID != H29_HYPOTHESIS_ID
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key["session_chain_rate"] == H29_HYPOTHESIS_ID
    assert by_key.get("ohlc_identity_rate") not in {
        H23_HYPOTHESIS_ID,
        H24_HYPOTHESIS_ID,
        H29_HYPOTHESIS_ID,
    }
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert ohlc_identity_vs_session_chain_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert (
        ohlc_identity_vs_session_chain_never_equate_honesty_errors({"ohlc_identity_rate": 1.0})
        == []
    )
