"""sweep_reject_control_diff_p ≠ gap_finite_rate — never-equate (H44↔gap)."""
...
def test_keys_and_h44_bind() -> None:
    assert "sweep_reject_control_diff_p" != "gap_finite_rate"
    assert H44_HYPOTHESIS_ID not in {
        H20_HYPOTHESIS_ID,
        H21_HYPOTHESIS_ID,
        H22_HYPOTHESIS_ID,
    }
    ...
    assert by_key.get("gap_finite_rate") != H44_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert sweep_reject_control_diff_p_vs_gap_finite_never_equate_honesty_errors(receipt) == []
