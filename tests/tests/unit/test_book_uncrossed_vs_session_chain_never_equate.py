"""book_uncrossed_rate ≠ session_chain_rate — never-equate (H21↔H29)."""
...
def test_keys_gates_and_h29_bind() -> None:
    assert "book_uncrossed_rate" != "session_chain_rate"
    assert H21_HYPOTHESIS_ID != H29_HYPOTHESIS_ID
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert book_uncrossed_vs_session_chain_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert (
        book_uncrossed_vs_session_chain_never_equate_honesty_errors({"book_uncrossed_rate": 1.0})
        == []
    )
