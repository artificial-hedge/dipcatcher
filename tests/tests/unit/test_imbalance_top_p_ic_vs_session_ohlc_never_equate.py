"""imbalance_top_p_ic ≠ session_ohlc_identity_rate — never-equate (H22)."""
...
def test_keys_gates_and_session_ohlc_unbound() -> None:
    assert "imbalance_top_p_ic" != "session_ohlc_identity_rate"
    by_key = {s[0]: s[1] for s in NORTHSET_H23_H28_SPECS}
    assert by_key.get("session_ohlc_identity_rate") not in {
        H23_HYPOTHESIS_ID,
        H24_HYPOTHESIS_ID,
        H29_HYPOTHESIS_ID,
        H22_HYPOTHESIS_ID,
    ...
    assert not northset_has_finite_imbalance_p_ic(
        {"session_ohlc_identity_rate": 1.0, "book_hypothesis_eligible": True}
    )
...
def test_synth_both_stamped_never_equate_clean() -> None:
    ...
    assert imbalance_top_p_ic_vs_session_ohlc_never_equate_honesty_errors(receipt) == []


def test_one_key_absent_skipped() -> None:
    assert (
        imbalance_top_p_ic_vs_session_ohlc_never_equate_honesty_errors({"imbalance_top_p_ic": 0.05})
        == []
    )
