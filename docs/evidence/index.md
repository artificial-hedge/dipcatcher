# Receipt-bound evidence

This page is generated by `scripts/build_evidence_report.py` from receipts
already committed in the repository. Do not edit it by hand. Regenerate
with `make evidence`.

Every number below is copied from a receipt field. The generator does not
rerun tournaments and does not derive new scores. A receipt field is omitted
when its name tokenizes to a forbidden research-headline metric, with two
exceptions: the boolean flag `live_pnl_claim` is always shown, and
parity-residual fields are shown with that token replaced by `<redacted>`.
Failed and blocked results stay visible. Disclaimer, limitation, and
known-difference strings are copied verbatim.

## Verification

Phase-1 receipts are sealed with `quant_fund.research.phase1_verify.verify_phase1_index`.
Notebook-shaped receipts are sealed with `quant_fund.research.verify.verify_research_artifact`.
Other committed receipts in this page have no embedded seal in those verifiers;
their file SHA-256 is the byte identity.

The verifier also attests this checkout's interpreter, package versions,
working-tree source bytes, dataset file bytes, and git object availability.
Those checks are not functions of receipt bytes, so their pass or fail text
is not copied into this page.

Index source: `data/metadata/research/phase1_evidence_index.json`
Index file sha256: `3e8d28e2ad08137d5e0ea7b8703600d710cdda37ebe83e384afc50fbc94d1f7a`
Index embedded seal: `0ce794b56249952fce5b2ff1046eea9e50b2f4e6d691539b8019959131873204`
Index git revision: `c564646b14849d72c5891a90034d8e26361fe0f5`
Index git worktree sha256: `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`
Benchmark catalog version: `2`

Content seals: pass

## Phase-1 forecast baselines

Sealed fixed-split forecast diagnostics on the Phase-1 US snapshot.
Lower date-equal-weight MSE is the receipt's scoring direction.
Dataset hash on each row is `protocol.dataset_sha256` from the benchmark
manifest when the file itself does not embed one. Git revision is the
index `git_revision` that binds the run.

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| data/metadata/real_benchmark/us_wide_20260925/manifest.json | b4b4ec2f1f41eec36862747f64697a175f30858d05f7753fcc6f766e98b979f8 | a6789ac23923e4e7f2fd1afe9a20f7c5dbaeb3666851c5e3708cf2c5dfa7d384 | pass | c564646b14849d72c5891a90034d8e26361fe0f5 | e22bf3eff634c742299b6495f8daf8f02adcc6cda47d9641e3094c0c191ae117 | real | absent | true | false |
| data/metadata/real_benchmark/us_wide_20260925/test.json | 2a9a4d5bf16a78ae396eaaa5f8cf89b03db45c8286795f4a1adc902b5d0e75f5 | d8449f6cc5f117589104e5e2c456797c547ed76daee9cf6ad1946fb594859b01 | pass | c564646b14849d72c5891a90034d8e26361fe0f5 | e22bf3eff634c742299b6495f8daf8f02adcc6cda47d9641e3094c0c191ae117 | real | false | true | false |
| data/metadata/real_benchmark/us_wide_20260925/validation.json | 73f2c74d1f7f62caa76f0ca2760badfa80bb5844d4d0069afb71ddbd8185f86e | 84af6ddc95a59a11452ea5eddad00d5ab4224dc67eb3de3e75b10707d021f93b | pass | c564646b14849d72c5891a90034d8e26361fe0f5 | e22bf3eff634c742299b6495f8daf8f02adcc6cda47d9641e3094c0c191ae117 | real | false | true | false |

Holdout status: `previously_inspected`
Names: `424`
Rows: `1132803`
Late rows: `0`
Excluded windows: `0`
Source labels: `yahoo`
Survivorship bias: `true`
Holdout previously inspected: `true`

| split | rows | dates |
| --- | --- | --- |
| train | 626297 | 1490 |
| validation | 208395 | 495 |
| test | 181451 | 431 |

| phase | model | date_equal_weight_mse | date_equal_weight_mae | n_rows | n_dates |
| --- | --- | --- | --- | --- | --- |
| validation | zero | 0.00042737707376421305 | 0.014535197485731068 | 208395 | 495 |
| validation | historical_mean | 0.00042775123473955026 | 0.014541446794625266 | 208395 | 495 |
| validation | rolling_mean_20 | 0.00044714530569201074 | 0.014945063658222476 | 208395 | 495 |
| validation | ridge | 0.00043012625721917177 | 0.014594179764524259 | 208395 | 495 |
| test | zero | 0.0003899407444036478 | 0.012870926140241257 | 181451 | 431 |
| test | historical_mean | 0.0003897374788546646 | 0.01285457126409479 | 181451 | 431 |
| test | rolling_mean_20 | 0.0004090936928422289 | 0.01332045812890271 | 181451 | 431 |
| test | ridge | 0.00038903174185071765 | 0.012853767095097215 | 181451 | 431 |

validation: ridge date-equal-weight MSE is higher than the zero baseline.
test: ridge date-equal-weight MSE is lower than the zero baseline.

Limitations copied verbatim from `data/metadata/real_benchmark/us_wide_20260925/manifest.json`:

<!-- verbatim-receipt-text -->
> Source, usage rights and historical availability are user declarations, not independently verified.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> File hashes detect changes; they do not prove a holdout was never inspected.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Close-to-close forecast scores do not establish executable net returns.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Input universe has survivorship bias.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Historical availability timestamps were reconstructed.
<!-- /verbatim-receipt-text -->

## Net-return tournament

Frozen equal-weight, momentum-20, and reversal-1 slate. Statistics below
are the sealed phase reports, including Reality Check, SPA, and StepM.
`total_return` and `mean_excess_net_return` are copied from those fields.
The receipt claim is `simulated_net_price_return_tournament`.

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| data/metadata/net_tournament/us_wide_20260925/manifest.json | a4a8f384c14b4a7805fbab5f52158a11a38aa78d87368a65c933acd061054eaf | 4650484a42ea0bbb926e46f24f1d85dc46016ad87585523dd5e26380e8d3bf13 | pass | c564646b14849d72c5891a90034d8e26361fe0f5 | e22bf3eff634c742299b6495f8daf8f02adcc6cda47d9641e3094c0c191ae117 | real | absent | true | false |
| data/metadata/net_tournament/us_wide_20260925/test.attempt.json | 1f87f6ca24f8787a812ab6f757ec1d6f5e2f96c805433b9c9def538bdbda9c9c | 8ce81e7db9ca714d4aa2808eee6ab7dcf39fe63e045dab4b3fc7ad7276abe603 | pass | c564646b14849d72c5891a90034d8e26361fe0f5 | e22bf3eff634c742299b6495f8daf8f02adcc6cda47d9641e3094c0c191ae117 | real | absent | absent | absent |
| data/metadata/net_tournament/us_wide_20260925/test.json.gz | 61f2a1cd147f5d418e190c685e2b7845e09e8a44746e64a3726797bea48b3efe | a7ea519e504eee2b2239fec69d163e7d8bbcd9b57bcbb2ddc49b74ea2211dbbc | pass | c564646b14849d72c5891a90034d8e26361fe0f5 | e22bf3eff634c742299b6495f8daf8f02adcc6cda47d9641e3094c0c191ae117 | real | false | true | false |
| data/metadata/net_tournament/us_wide_20260925/validation.attempt.json | d4ffa0198f2bab8da516a96bb4829726d1e3ef900b015cd5a010117b28f98ff2 | 0ee332877df548868fe45f71f2cb20a9ef01600d7e65476acb79a49b43a7495b | pass | c564646b14849d72c5891a90034d8e26361fe0f5 | e22bf3eff634c742299b6495f8daf8f02adcc6cda47d9641e3094c0c191ae117 | real | absent | absent | absent |
| data/metadata/net_tournament/us_wide_20260925/validation.json.gz | abf8037e85987750fcc6cff42b53137464612e0409e97642e64a43ec961ece36 | c2be98ce3e855af08819d7273b7df6b5c087a7b5107b57535ad8437a07d692c7 | pass | c564646b14849d72c5891a90034d8e26361fe0f5 | e22bf3eff634c742299b6495f8daf8f02adcc6cda47d9641e3094c0c191ae117 | real | false | true | false |

| phase | selected | complete | economic_evidence_gate | selected_holdout_adjusted_rejection | selected_survives_double_impact | all_terminal_liquidations_complete | promote | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| validation | momentum_20 | true | false | false | false | true | false | false |
| test | momentum_20 | true | false | false | false | true | false | false |

| phase | scenario | candidate | status | total_return | rejected_orders | max_drawdown | liquidation_complete | error |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| validation | configured | equal_weight | completed | 0.02216660701224038 | 0 | -0.2885666615405219 | true | null |
| validation | configured | momentum_20 | completed | -0.022558444727763027 | 1 | -0.3003612892122214 | true | null |
| validation | configured | reversal_1 | completed | -0.2886898757053019 | 29 | -0.42261728316245106 | true | null |
| validation | double_impact | equal_weight | completed | 0.022114647860794756 | 0 | -0.28857943061586666 | true | null |
| validation | double_impact | momentum_20 | completed | -0.02321380237336579 | 1 | -0.3005575489011997 | true | null |
| validation | double_impact | reversal_1 | completed | -0.2906796444103349 | 29 | -0.4233216519448494 | true | null |
| test | configured | equal_weight | completed | 0.36556893586625994 | 0 | -0.19978459471810428 | true | null |
| test | configured | momentum_20 | completed | 0.2273650849289024 | 4 | -0.23328072728023874 | true | null |
| test | configured | reversal_1 | completed | 0.009488889358648667 | 23 | -0.2498030476428128 | true | null |
| test | double_impact | equal_weight | completed | 0.3655121653187692 | 0 | -0.19978737936498825 | true | null |
| test | double_impact | momentum_20 | completed | 0.22664341344131445 | 4 | -0.23336947593795465 | true | null |
| test | double_impact | reversal_1 | completed | 0.006815549469270765 | 23 | -0.25075860192318267 | true | null |

| phase | scenario | comparison_status | candidate | n_dates | reality_check_p | spa_consistent_p | stepm_adjusted_p | mean_excess_net_return |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| validation | configured | tested | momentum_20 | 495 | 0.8790604697651174 | 1.0 | 0.8700649675162418 | -7.195601417700061e-05 |
| validation | configured | tested | reversal_1 | 495 | 0.8790604697651174 | 1.0 | 0.9995002498750625 | -0.0006917326649306404 |
| validation | double_impact | tested | momentum_20 | 495 | 0.88055972013993 | 1.0 | 0.8700649675162418 | -7.320784724892539e-05 |
| validation | double_impact | tested | reversal_1 | 495 | 0.88055972013993 | 1.0 | 0.9995002498750625 | -0.0006972899111765339 |
| test | configured | tested | momentum_20 | 431 | 0.9455272363818091 | 1.0 | 0.9505247376311844 | -0.00022858234130943662 |
| test | configured | tested | reversal_1 | 431 | 0.9455272363818091 | 1.0 | 0.9820089955022488 | -0.000673693479211616 |
| test | double_impact | tested | momentum_20 | 431 | 0.9465267366316842 | 1.0 | 0.9510244877561219 | -0.00022985028509299856 |
| test | double_impact | tested | reversal_1 | 431 | 0.9465267366316842 | 1.0 | 0.9820089955022488 | -0.0006797536284945887 |

Limitations copied verbatim from `data/metadata/net_tournament/us_wide_20260925/validation.json.gz`:

<!-- verbatim-receipt-text -->
> Source, usage rights and historical availability are user declarations, not independently verified.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> File hashes detect changes; they do not prove a holdout was never inspected.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Close-to-close forecast scores do not establish executable net returns.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Input universe has survivorship bias.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Historical availability timestamps were reconstructed.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Daily simulated price returns; consistent open/close share units must be verified upstream.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Dividends, corporate actions and real borrow locates are not modeled. APRs are declared scenario inputs.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Trades execute at next session open with modeled costs; opening-auction depth is not observed.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Gross limits are shared; long-only and long-short books can have different factor/net exposures.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Trial control covers this frozen slate, not undisclosed experiments in other run directories.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Allocation return/uncertainty inputs are unvalidated proxies; pair ablations are descriptive.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Impact stress changes realized costs; allocation planning coefficients stay frozen.
<!-- /verbatim-receipt-text -->

## Cost-aware construction

Status: FAILED / BLOCKED

Recorded failure reasons:

- `configured` / `momentum_20_cost_aware`: `allocation failed: optimal_inaccurate`
- `configured` / `reversal_1_cost_aware`: `allocation failed: optimal_inaccurate`
- `double_impact` / `momentum_20_cost_aware`: `allocation failed: optimal_inaccurate`
- `double_impact` / `reversal_1_cost_aware`: `allocation failed: optimal_inaccurate`

Index `test_sha256` for this run: `null`.
No test receipt is rendered because that seal is null.

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| data/metadata/cost_aware_tournament/us_wide_20260925/manifest.json | ea26eaf805da7963eea40d81ffb69078199d3695bed6669b281a38947160a763 | 7dab6ee1eb6b39c47e85f048c8f9584d5c6b08f214ef3c6a4cb908228be7ca90 | pass | c564646b14849d72c5891a90034d8e26361fe0f5 | e22bf3eff634c742299b6495f8daf8f02adcc6cda47d9641e3094c0c191ae117 | real | absent | true | false |
| data/metadata/cost_aware_tournament/us_wide_20260925/validation.attempt.json | 1fe173b736824f8372d5e94c7c09cb7f8d30c71b4020796db3802babfca3efa5 | b9a680e45d664b103cd5c37b191cb8bcdbf5ca0210277aad1de50d69b120c4a6 | pass | c564646b14849d72c5891a90034d8e26361fe0f5 | e22bf3eff634c742299b6495f8daf8f02adcc6cda47d9641e3094c0c191ae117 | real | absent | absent | absent |
| data/metadata/cost_aware_tournament/us_wide_20260925/validation.json.gz | 67c1a4738e479fb3c54a95d422e1e32e9a276c7dc63d1c876f554e0fc50f4273 | 972af75021460e68c463f6d02fbdd879a0f053a4beb15ff800569e1fda35cc71 | pass | c564646b14849d72c5891a90034d8e26361fe0f5 | e22bf3eff634c742299b6495f8daf8f02adcc6cda47d9641e3094c0c191ae117 | real | false | true | false |

| phase | selected | complete | economic_evidence_gate | selected_holdout_adjusted_rejection | selected_survives_double_impact | all_terminal_liquidations_complete | promote | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| validation | null | false | false | false | false | false | false | false |

| phase | scenario | candidate | status | total_return | rejected_orders | max_drawdown | liquidation_complete | error |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| validation | configured | equal_weight | completed | 0.02216660701224038 | 0 | -0.2885666615405219 | true | null |
| validation | configured | momentum_20 | completed | -0.022558444727763027 | 1 | -0.3003612892122214 | true | null |
| validation | configured | reversal_1 | completed | -0.2886898757053019 | 29 | -0.42261728316245106 | true | null |
| validation | configured | momentum_20_cost_aware | failed | null | null | null | null | allocation failed: optimal_inaccurate |
| validation | configured | reversal_1_cost_aware | failed | null | null | null | null | allocation failed: optimal_inaccurate |
| validation | double_impact | equal_weight | completed | 0.022114647860794756 | 0 | -0.28857943061586666 | true | null |
| validation | double_impact | momentum_20 | completed | -0.02321380237336579 | 1 | -0.3005575489011997 | true | null |
| validation | double_impact | reversal_1 | completed | -0.2906796444103349 | 29 | -0.4233216519448494 | true | null |
| validation | double_impact | momentum_20_cost_aware | failed | null | null | null | null | allocation failed: optimal_inaccurate |
| validation | double_impact | reversal_1_cost_aware | failed | null | null | null | null | allocation failed: optimal_inaccurate |

| phase | scenario | comparison_status | candidate | n_dates | reality_check_p | spa_consistent_p | stepm_adjusted_p | mean_excess_net_return |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| validation | configured | incomplete_trials | momentum_20 | null | null | null | null | null |
| validation | configured | incomplete_trials | reversal_1 | null | null | null | null | null |
| validation | configured | incomplete_trials | momentum_20_cost_aware | null | null | null | null | null |
| validation | configured | incomplete_trials | reversal_1_cost_aware | null | null | null | null | null |
| validation | double_impact | incomplete_trials | momentum_20 | null | null | null | null | null |
| validation | double_impact | incomplete_trials | reversal_1 | null | null | null | null | null |
| validation | double_impact | incomplete_trials | momentum_20_cost_aware | null | null | null | null | null |
| validation | double_impact | incomplete_trials | reversal_1_cost_aware | null | null | null | null | null |

Solver diagnostics copied from failed trials:

| phase | scenario | candidate | solver | status | iterations | solve_time_seconds | weights_accepted |
| --- | --- | --- | --- | --- | --- | --- | --- |
| validation | configured | momentum_20_cost_aware | CLARABEL | optimal_inaccurate | 28 | 0.002386201 | false |
| validation | configured | reversal_1_cost_aware | CLARABEL | optimal_inaccurate | 48 | 0.006821058 | false |
| validation | double_impact | momentum_20_cost_aware | CLARABEL | optimal_inaccurate | 28 | 0.002256059 | false |
| validation | double_impact | reversal_1_cost_aware | CLARABEL | optimal_inaccurate | 27 | 0.003224641 | false |

Absent phase reports: `test`.

Limitations copied verbatim from `data/metadata/cost_aware_tournament/us_wide_20260925/validation.json.gz`:

<!-- verbatim-receipt-text -->
> Source, usage rights and historical availability are user declarations, not independently verified.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> File hashes detect changes; they do not prove a holdout was never inspected.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Close-to-close forecast scores do not establish executable net returns.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Input universe has survivorship bias.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Historical availability timestamps were reconstructed.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Daily simulated price returns; consistent open/close share units must be verified upstream.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Dividends, corporate actions and real borrow locates are not modeled. APRs are declared scenario inputs.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Trades execute at next session open with modeled costs; opening-auction depth is not observed.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Gross limits are shared; long-only and long-short books can have different factor/net exposures.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Trial control covers this frozen slate, not undisclosed experiments in other run directories.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Allocation return/uncertainty inputs are unvalidated proxies; pair ablations are descriptive.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> Impact stress changes realized costs; allocation planning coefficients stay frozen.
<!-- /verbatim-receipt-text -->

| scenario | candidate | control | status | inference |
| --- | --- | --- | --- | --- |
| configured | momentum_20_cost_aware | momentum_20 | incomplete_pair | descriptive_only |
| configured | reversal_1_cost_aware | reversal_1 | incomplete_pair | descriptive_only |
| double_impact | momentum_20_cost_aware | momentum_20 | incomplete_pair | descriptive_only |
| double_impact | reversal_1_cost_aware | reversal_1 | incomplete_pair | descriptive_only |

## qlib incumbent parity

Matched-workload comparison against qlib. Terminal account levels whose
field names tokenize to a forbidden research-headline metric are omitted.
Parity residuals from the correctness object are copied under redacted keys.

### `receipts/incumbent_bench_qlib.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| receipts/incumbent_bench_qlib.json | f455123351b44151d68876d1a93fa3a2dc449c51d280ee83926f22cd9861984f | absent | no_embedded_seal | absent | workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb | real | absent | true | false |

Incumbent: `qlib` `0.9.7`
Schema: `incumbent_bench.v1`
Recorded environment: `platform`=`darwin`, `python`=`3.12.13`

| field | value |
| --- | --- |
| common_dates | 997 |
| dipcatcher_total_fees | 131846.29515885914 |
| fills_dipcatcher | 1587 |
| <redacted>_max_abs_diff | 0.1783271913882345 |
| <redacted>_max_rel_diff | 1.0290734772388363e-07 |
| qlib_order_days | 670 |
| qlib_total_cost | 131833.36603253405 |

| latency field | value |
| --- | --- |
| reps | 5 |
| dipcatcher_ms_median | 101.3606249998702 |
| qlib_ms_median | 10579.349499999807 |
| dipcatcher_ms_all | 105.75, 101.36, 97.17, 99.12, 103.04 |
| qlib_ms_all | 9469.53, 10510.51, 18078.94, 16197.97, 10579.35 |

Workload fields copied from the receipt:

- `assets`:
  - BTCUSDT
  - ETHUSDT
  - SOLUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `qlib_settings`:
  - `deal_price`: $open
  - `limit_threshold`: 1000000000.0
  - `min_cost`: 0.0
  - `risk_degree`: 1.0
  - `trade_unit`: null
  - `volume_threshold`: null
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 0.9/3 per gated name
- `weight_scale`: 1.0

Omitted 3 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `receipts/incumbent_bench_qlib.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs qlib 0.9.x on real Binance daily bars. Correctness is NAV parity; latency is single-process wall time. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `receipts/incumbent_bench_qlib.json`:

<!-- verbatim-receipt-text -->
> qlib quotes are float32 (.bin format); dipcatcher uses float64 parquet prices. Residual NAV divergence is f32 price quantization (~1e-7 rel, ~$0.18 worst day on ~$1.5M).
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> qlib order generation sells-down before buys within a step and clips buy amounts to cash+cost; dipcatcher rejects a whole order that would overdraw. Workload keeps a 0.9 weight-sum buffer so neither policy binds.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> qlib default order generators renormalize weights to sum=1 and floor amounts to integer units; a custom OrderGenerator (MatchedOrderGen) applies dipcatcher's exact sizing target_units = w * NAV / open.
<!-- /verbatim-receipt-text -->

<!-- verbatim-receipt-text -->
> limit_threshold=None does NOT disable qlib's price-limit halt: it falls back to C.limit_threshold (0.095 for region=cn) and silently drops orders on >9.5% daily moves. limit_threshold=1e9 disables it. Without this, NAV diverged ~14% on this workload.
<!-- /verbatim-receipt-text -->

## vectorbt comparisons

Matched-workload comparisons against vectorbt. One block per committed
evidence file. Terminal account levels whose field names tokenize to a
forbidden research-headline metric are omitted. Parity residuals from
the correctness object are copied under redacted keys.

### `.dsh-24x7/evidence-incumbent-vectorbt-11a-fast.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt-11a-fast.json | 820b44bd2c12e8a240babeec0f84c3c661f5f6abba70bcc58e96567999cf6a78 | absent | no_embedded_seal | absent | workload.bar_files_sha256.adausdt_1d.parquet=59b5ec1a9076065bbedc3e14e49e59ee5f3781646721710dffca0aeab093a3f2, workload.bar_files_sha256.avaxusdt_1d.parquet=40028d2c07bced6cc2d37b6629538959f99fabc8b6653c8c696d87c0a7cc2359, workload.bar_files_sha256.bnbusdt_1d.parquet=822ec14bb30fda27bd14b4ab2a99070ae2d78858794c08444f8c0adbf75a80c1, workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.dogeusdt_1d.parquet=a8a8005ee958a265c313ac6ee3da9f0dacf1e1d7be95b7496bb5a58d687bf7c7, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.linkusdt_1d.parquet=e09b987e4a7c9fbb9b8fe659d68677d75fe687b963451b09f14698dbe997b512, workload.bar_files_sha256.ltcusdt_1d.parquet=ce5bd0ab9540792895940afae9e0934db0eaf0e221bf3ae7bd3d1e89ce361b3a, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb, workload.bar_files_sha256.trxusdt_1d.parquet=9eb8c62328057e0eef19013be9e435b585d880a9ebe14a6f59bc1cc14427d3f3, workload.bar_files_sha256.xrpusdt_1d.parquet=d03430a83a62632adaa02878069c6124f9855247238fc2f635d19b0c6bf91f30 | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded engine: `fast`
Recorded environment: `platform`=`win32`, `python`=`3.12.14`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| dipcatcher_total_fees | 159731.38989095474 |
| fills_dipcatcher | 5734 |
| <redacted>_max_abs_diff | 1.862645149230957e-09 |
| <redacted>_max_rel_diff | 1.5354674690941092e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 5734 |
| vectorbt_total_fees | 159731.38989095468 |

| latency field | value |
| --- | --- |
| reps | 10 |
| dipcatcher_ms_median | 490.6680999993114 |
| vectorbt_ms_median | 231.77319999376778 |
| dipcatcher_ms_all | 456.13, 656.83, 771.18, 613.84, 559.73, 402.65, 472.56, 508.78, 448.7, 468.13 |
| vectorbt_ms_all | 218.94, 200.2, 236.34, 251.63, 252.25, 239.29, 238.83, 180.37, 178.54, 227.2 |

Workload fields copied from the receipt:

- `assets`:
  - ADAUSDT
  - AVAXUSDT
  - BNBUSDT
  - BTCUSDT
  - DOGEUSDT
  - ETHUSDT
  - LINKUSDT
  - LTCUSDT
  - SOLUSDT
  - TRXUSDT
  - XRPUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/11 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 5106
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: ADAUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1397582.39
  - `stale_marks`: accepted; equity NaN rows=0, final=1443150.7517163786

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-11a-fast.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-11a-fast.json`:

<!-- verbatim-receipt-text -->
> cash constraint: with weights summing to exactly 1.0, dipcatcher rejects whole orders that would overdraw cash (329 rejects on the 3-asset workload), while vectorbt clips/fills; workloads use a 0.9 target-sum buffer so the parity test isolates accounting.
<!-- /verbatim-receipt-text -->

### `.dsh-24x7/evidence-incumbent-vectorbt-11a.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt-11a.json | 01e792f65ee74dd34b284be1f08a7eea740bbf2c347f455680e2748ccf07066a | absent | no_embedded_seal | absent | workload.bar_files_sha256.adausdt_1d.parquet=59b5ec1a9076065bbedc3e14e49e59ee5f3781646721710dffca0aeab093a3f2, workload.bar_files_sha256.avaxusdt_1d.parquet=40028d2c07bced6cc2d37b6629538959f99fabc8b6653c8c696d87c0a7cc2359, workload.bar_files_sha256.bnbusdt_1d.parquet=822ec14bb30fda27bd14b4ab2a99070ae2d78858794c08444f8c0adbf75a80c1, workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.dogeusdt_1d.parquet=a8a8005ee958a265c313ac6ee3da9f0dacf1e1d7be95b7496bb5a58d687bf7c7, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.linkusdt_1d.parquet=e09b987e4a7c9fbb9b8fe659d68677d75fe687b963451b09f14698dbe997b512, workload.bar_files_sha256.ltcusdt_1d.parquet=ce5bd0ab9540792895940afae9e0934db0eaf0e221bf3ae7bd3d1e89ce361b3a, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb, workload.bar_files_sha256.trxusdt_1d.parquet=9eb8c62328057e0eef19013be9e435b585d880a9ebe14a6f59bc1cc14427d3f3, workload.bar_files_sha256.xrpusdt_1d.parquet=d03430a83a62632adaa02878069c6124f9855247238fc2f635d19b0c6bf91f30 | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded environment: `platform`=`darwin`, `python`=`3.12.13`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| dipcatcher_total_fees | 159731.38989095474 |
| fills_dipcatcher | 5734 |
| <redacted>_max_abs_diff | 1.862645149230957e-09 |
| <redacted>_max_rel_diff | 1.5354674690941092e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 5734 |
| vectorbt_total_fees | 159731.38989095468 |

| latency field | value |
| --- | --- |
| reps | 7 |
| dipcatcher_ms_median | 446.48379099999147 |
| vectorbt_ms_median | 82.67508300014015 |
| dipcatcher_ms_all | 485.59, 395.43, 381.44, 435.2, 452.79, 446.48, 552.2 |
| vectorbt_ms_all | 82.68, 75.68, 74.3, 87.85, 120.01, 79.36, 150.77 |

Workload fields copied from the receipt:

- `assets`:
  - ADAUSDT
  - AVAXUSDT
  - BNBUSDT
  - BTCUSDT
  - DOGEUSDT
  - ETHUSDT
  - LINKUSDT
  - LTCUSDT
  - SOLUSDT
  - TRXUSDT
  - XRPUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/11 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 5106
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: ADAUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1397582.39
  - `stale_marks`: accepted; equity NaN rows=0, final=1443150.7517163786

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-11a.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-11a.json`:

<!-- verbatim-receipt-text -->
> cash constraint: with weights summing to exactly 1.0, dipcatcher rejects whole orders that would overdraw cash (329 rejects on the 3-asset workload), while vectorbt clips/fills; workloads use a 0.9 target-sum buffer so the parity test isolates accounting.
<!-- /verbatim-receipt-text -->

### `.dsh-24x7/evidence-incumbent-vectorbt-11asset.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt-11asset.json | 776d8306df65497cc4efad2d0ccef694801cc61670d0c36c04daa1bc073c339c | absent | no_embedded_seal | absent | workload.bar_files_sha256.adausdt_1d.parquet=59b5ec1a9076065bbedc3e14e49e59ee5f3781646721710dffca0aeab093a3f2, workload.bar_files_sha256.avaxusdt_1d.parquet=40028d2c07bced6cc2d37b6629538959f99fabc8b6653c8c696d87c0a7cc2359, workload.bar_files_sha256.bnbusdt_1d.parquet=822ec14bb30fda27bd14b4ab2a99070ae2d78858794c08444f8c0adbf75a80c1, workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.dogeusdt_1d.parquet=a8a8005ee958a265c313ac6ee3da9f0dacf1e1d7be95b7496bb5a58d687bf7c7, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.linkusdt_1d.parquet=e09b987e4a7c9fbb9b8fe659d68677d75fe687b963451b09f14698dbe997b512, workload.bar_files_sha256.ltcusdt_1d.parquet=ce5bd0ab9540792895940afae9e0934db0eaf0e221bf3ae7bd3d1e89ce361b3a, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb, workload.bar_files_sha256.trxusdt_1d.parquet=9eb8c62328057e0eef19013be9e435b585d880a9ebe14a6f59bc1cc14427d3f3, workload.bar_files_sha256.xrpusdt_1d.parquet=d03430a83a62632adaa02878069c6124f9855247238fc2f635d19b0c6bf91f30 | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded environment: `platform`=`darwin`, `python`=`3.12.13`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| fills_dipcatcher | 5734 |
| <redacted>_max_abs_diff | 1.862645149230957e-09 |
| <redacted>_max_rel_diff | 1.5354674690941092e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 5734 |
| vectorbt_total_fees | 159731.38989095468 |

| latency field | value |
| --- | --- |
| reps | 10 |
| dipcatcher_ms_median | 1066.24925000051 |
| vectorbt_ms_median | 20.310458499807282 |
| dipcatcher_ms_all | 911.85, 1006.32, 1000.19, 971.57, 1279.66, 1433.67, 1304.92, 1126.18, 1310.02, 983.31 |
| vectorbt_ms_all | 24.25, 20.41, 20.58, 20.21, 20.0, 20.67, 20.88, 19.53, 18.66, 19.06 |

Workload fields copied from the receipt:

- `assets`:
  - ADAUSDT
  - AVAXUSDT
  - BNBUSDT
  - BTCUSDT
  - DOGEUSDT
  - ETHUSDT
  - LINKUSDT
  - LTCUSDT
  - SOLUSDT
  - TRXUSDT
  - XRPUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/11 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 5106
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: ADAUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1397582.39
  - `stale_marks`: accepted; equity NaN rows=0, final=1443150.7517163786

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-11asset.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-11asset.json`:

Known semantic differences: absent

### `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a-numba.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt-fast-11a-numba.json | 88d27b69a9ec2a92ff69aa9bc908ce1ef85008f44ec1557f3de2336a032fb6b4 | absent | no_embedded_seal | absent | workload.bar_files_sha256.adausdt_1d.parquet=59b5ec1a9076065bbedc3e14e49e59ee5f3781646721710dffca0aeab093a3f2, workload.bar_files_sha256.avaxusdt_1d.parquet=40028d2c07bced6cc2d37b6629538959f99fabc8b6653c8c696d87c0a7cc2359, workload.bar_files_sha256.bnbusdt_1d.parquet=822ec14bb30fda27bd14b4ab2a99070ae2d78858794c08444f8c0adbf75a80c1, workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.dogeusdt_1d.parquet=a8a8005ee958a265c313ac6ee3da9f0dacf1e1d7be95b7496bb5a58d687bf7c7, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.linkusdt_1d.parquet=e09b987e4a7c9fbb9b8fe659d68677d75fe687b963451b09f14698dbe997b512, workload.bar_files_sha256.ltcusdt_1d.parquet=ce5bd0ab9540792895940afae9e0934db0eaf0e221bf3ae7bd3d1e89ce361b3a, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb, workload.bar_files_sha256.trxusdt_1d.parquet=9eb8c62328057e0eef19013be9e435b585d880a9ebe14a6f59bc1cc14427d3f3, workload.bar_files_sha256.xrpusdt_1d.parquet=d03430a83a62632adaa02878069c6124f9855247238fc2f635d19b0c6bf91f30 | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded engine: `fast`
Recorded environment: `platform`=`win32`, `python`=`3.12.14`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| dipcatcher_total_fees | 159731.38989095474 |
| fills_dipcatcher | 5734 |
| <redacted>_max_abs_diff | 1.862645149230957e-09 |
| <redacted>_max_rel_diff | 1.5354674690941092e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 5734 |
| vectorbt_total_fees | 159731.38989095468 |

| latency field | value |
| --- | --- |
| reps | 10 |
| dipcatcher_ms_median | 84.80179998150561 |
| vectorbt_ms_median | 76.57855001161806 |
| dipcatcher_ms_all | 76.6, 75.95, 101.67, 77.35, 97.95, 79.31, 90.29, 97.57, 104.12, 78.5 |
| vectorbt_ms_all | 73.42, 71.41, 79.2, 70.29, 80.42, 73.96, 71.92, 80.69, 79.38, 79.71 |

Workload fields copied from the receipt:

- `assets`:
  - ADAUSDT
  - AVAXUSDT
  - BNBUSDT
  - BTCUSDT
  - DOGEUSDT
  - ETHUSDT
  - LINKUSDT
  - LTCUSDT
  - SOLUSDT
  - TRXUSDT
  - XRPUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/11 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 5106
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: ADAUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1397582.39
  - `stale_marks`: accepted; equity NaN rows=0, final=1443150.7517163786

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a-numba.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a-numba.json`:

<!-- verbatim-receipt-text -->
> cash constraint: with weights summing to exactly 1.0, dipcatcher rejects whole orders that would overdraw cash (329 rejects on the 3-asset workload), while vectorbt clips/fills; workloads use a 0.9 target-sum buffer so the parity test isolates accounting.
<!-- /verbatim-receipt-text -->

### `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a-quiet.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt-fast-11a-quiet.json | 78a714819b26d7f9c08ef0277b8f35dfaf556f9f4422eda926296fc3aaad3cde | absent | no_embedded_seal | absent | workload.bar_files_sha256.adausdt_1d.parquet=59b5ec1a9076065bbedc3e14e49e59ee5f3781646721710dffca0aeab093a3f2, workload.bar_files_sha256.avaxusdt_1d.parquet=40028d2c07bced6cc2d37b6629538959f99fabc8b6653c8c696d87c0a7cc2359, workload.bar_files_sha256.bnbusdt_1d.parquet=822ec14bb30fda27bd14b4ab2a99070ae2d78858794c08444f8c0adbf75a80c1, workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.dogeusdt_1d.parquet=a8a8005ee958a265c313ac6ee3da9f0dacf1e1d7be95b7496bb5a58d687bf7c7, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.linkusdt_1d.parquet=e09b987e4a7c9fbb9b8fe659d68677d75fe687b963451b09f14698dbe997b512, workload.bar_files_sha256.ltcusdt_1d.parquet=ce5bd0ab9540792895940afae9e0934db0eaf0e221bf3ae7bd3d1e89ce361b3a, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb, workload.bar_files_sha256.trxusdt_1d.parquet=9eb8c62328057e0eef19013be9e435b585d880a9ebe14a6f59bc1cc14427d3f3, workload.bar_files_sha256.xrpusdt_1d.parquet=d03430a83a62632adaa02878069c6124f9855247238fc2f635d19b0c6bf91f30 | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded engine: `fast`
Recorded environment: `platform`=`win32`, `python`=`3.12.14`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| dipcatcher_total_fees | 159731.38989095474 |
| fills_dipcatcher | 5734 |
| <redacted>_max_abs_diff | 1.862645149230957e-09 |
| <redacted>_max_rel_diff | 1.5354674690941092e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 5734 |
| vectorbt_total_fees | 159731.38989095468 |

| latency field | value |
| --- | --- |
| reps | 7 |
| dipcatcher_ms_median | 96.0798000160139 |
| vectorbt_ms_median | 88.07540000998415 |
| dipcatcher_ms_all | 103.47, 88.61, 96.08, 91.96, 103.13, 76.17, 118.5 |
| vectorbt_ms_all | 89.6, 71.83, 88.82, 88.59, 87.06, 84.99, 88.08 |

Workload fields copied from the receipt:

- `assets`:
  - ADAUSDT
  - AVAXUSDT
  - BNBUSDT
  - BTCUSDT
  - DOGEUSDT
  - ETHUSDT
  - LINKUSDT
  - LTCUSDT
  - SOLUSDT
  - TRXUSDT
  - XRPUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/11 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 5106
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: ADAUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1397582.39
  - `stale_marks`: accepted; equity NaN rows=0, final=1443150.7517163786

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a-quiet.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a-quiet.json`:

<!-- verbatim-receipt-text -->
> cash constraint: with weights summing to exactly 1.0, dipcatcher rejects whole orders that would overdraw cash (329 rejects on the 3-asset workload), while vectorbt clips/fills; workloads use a 0.9 target-sum buffer so the parity test isolates accounting.
<!-- /verbatim-receipt-text -->

### `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a-v2.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt-fast-11a-v2.json | 4ec7758079ca7f4af1594e8af738906816fd7442202c6d32e9a70814e1a07418 | absent | no_embedded_seal | absent | workload.bar_files_sha256.adausdt_1d.parquet=59b5ec1a9076065bbedc3e14e49e59ee5f3781646721710dffca0aeab093a3f2, workload.bar_files_sha256.avaxusdt_1d.parquet=40028d2c07bced6cc2d37b6629538959f99fabc8b6653c8c696d87c0a7cc2359, workload.bar_files_sha256.bnbusdt_1d.parquet=822ec14bb30fda27bd14b4ab2a99070ae2d78858794c08444f8c0adbf75a80c1, workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.dogeusdt_1d.parquet=a8a8005ee958a265c313ac6ee3da9f0dacf1e1d7be95b7496bb5a58d687bf7c7, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.linkusdt_1d.parquet=e09b987e4a7c9fbb9b8fe659d68677d75fe687b963451b09f14698dbe997b512, workload.bar_files_sha256.ltcusdt_1d.parquet=ce5bd0ab9540792895940afae9e0934db0eaf0e221bf3ae7bd3d1e89ce361b3a, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb, workload.bar_files_sha256.trxusdt_1d.parquet=9eb8c62328057e0eef19013be9e435b585d880a9ebe14a6f59bc1cc14427d3f3, workload.bar_files_sha256.xrpusdt_1d.parquet=d03430a83a62632adaa02878069c6124f9855247238fc2f635d19b0c6bf91f30 | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded engine: `fast`
Recorded environment: `platform`=`win32`, `python`=`3.12.14`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| dipcatcher_total_fees | 159731.38989095474 |
| fills_dipcatcher | 5734 |
| <redacted>_max_abs_diff | 1.862645149230957e-09 |
| <redacted>_max_rel_diff | 1.5354674690941092e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 5734 |
| vectorbt_total_fees | 159731.38989095468 |

| latency field | value |
| --- | --- |
| reps | 10 |
| dipcatcher_ms_median | 1377.4473500088789 |
| vectorbt_ms_median | 289.9010000110138 |
| dipcatcher_ms_all | 1341.08, 1470.47, 1694.92, 1018.72, 1527.1, 1413.81, 1203.44, 1137.62, 1306.71, 2159.35 |
| vectorbt_ms_all | 289.21, 315.85, 236.62, 317.22, 340.2, 290.59, 216.36, 253.74, 275.54, 342.08 |

Workload fields copied from the receipt:

- `assets`:
  - ADAUSDT
  - AVAXUSDT
  - BNBUSDT
  - BTCUSDT
  - DOGEUSDT
  - ETHUSDT
  - LINKUSDT
  - LTCUSDT
  - SOLUSDT
  - TRXUSDT
  - XRPUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/11 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 5106
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: ADAUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1397582.39
  - `stale_marks`: accepted; equity NaN rows=0, final=1443150.7517163786

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a-v2.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a-v2.json`:

<!-- verbatim-receipt-text -->
> cash constraint: with weights summing to exactly 1.0, dipcatcher rejects whole orders that would overdraw cash (329 rejects on the 3-asset workload), while vectorbt clips/fills; workloads use a 0.9 target-sum buffer so the parity test isolates accounting.
<!-- /verbatim-receipt-text -->

### `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt-fast-11a.json | 1c2c073cd9b7c65b13a1b3c71c720f106a4c79f0b8ab8c740e5596105e1c935b | absent | no_embedded_seal | absent | workload.bar_files_sha256.adausdt_1d.parquet=59b5ec1a9076065bbedc3e14e49e59ee5f3781646721710dffca0aeab093a3f2, workload.bar_files_sha256.avaxusdt_1d.parquet=40028d2c07bced6cc2d37b6629538959f99fabc8b6653c8c696d87c0a7cc2359, workload.bar_files_sha256.bnbusdt_1d.parquet=822ec14bb30fda27bd14b4ab2a99070ae2d78858794c08444f8c0adbf75a80c1, workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.dogeusdt_1d.parquet=a8a8005ee958a265c313ac6ee3da9f0dacf1e1d7be95b7496bb5a58d687bf7c7, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.linkusdt_1d.parquet=e09b987e4a7c9fbb9b8fe659d68677d75fe687b963451b09f14698dbe997b512, workload.bar_files_sha256.ltcusdt_1d.parquet=ce5bd0ab9540792895940afae9e0934db0eaf0e221bf3ae7bd3d1e89ce361b3a, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb, workload.bar_files_sha256.trxusdt_1d.parquet=9eb8c62328057e0eef19013be9e435b585d880a9ebe14a6f59bc1cc14427d3f3, workload.bar_files_sha256.xrpusdt_1d.parquet=d03430a83a62632adaa02878069c6124f9855247238fc2f635d19b0c6bf91f30 | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded engine: `fast`
Recorded environment: `platform`=`win32`, `python`=`3.12.14`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| dipcatcher_total_fees | 159731.38989095474 |
| fills_dipcatcher | 5734 |
| <redacted>_max_abs_diff | 1.862645149230957e-09 |
| <redacted>_max_rel_diff | 1.5354674690941092e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 5734 |
| vectorbt_total_fees | 159731.38989095468 |

| latency field | value |
| --- | --- |
| reps | 10 |
| dipcatcher_ms_median | 732.7248000074178 |
| vectorbt_ms_median | 164.64749998704065 |
| dipcatcher_ms_all | 715.6, 747.76, 747.97, 899.94, 699.03, 717.69, 716.33, 765.94, 621.38, 913.49 |
| vectorbt_ms_all | 117.36, 123.35, 183.1, 122.32, 85.69, 227.66, 160.3, 202.07, 168.99, 219.9 |

Workload fields copied from the receipt:

- `assets`:
  - ADAUSDT
  - AVAXUSDT
  - BNBUSDT
  - BTCUSDT
  - DOGEUSDT
  - ETHUSDT
  - LINKUSDT
  - LTCUSDT
  - SOLUSDT
  - TRXUSDT
  - XRPUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/11 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 5106
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: ADAUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1397582.39
  - `stale_marks`: accepted; equity NaN rows=0, final=1443150.7517163786

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-11a.json`:

<!-- verbatim-receipt-text -->
> cash constraint: with weights summing to exactly 1.0, dipcatcher rejects whole orders that would overdraw cash (329 rejects on the 3-asset workload), while vectorbt clips/fills; workloads use a 0.9 target-sum buffer so the parity test isolates accounting.
<!-- /verbatim-receipt-text -->

### `.dsh-24x7/evidence-incumbent-vectorbt-fast-3a-numba.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt-fast-3a-numba.json | 6d4d32834f3d9ee204673980c44dbbaeafcc8f1884abb164b1b62e37f85708a5 | absent | no_embedded_seal | absent | workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded engine: `fast`
Recorded environment: `platform`=`win32`, `python`=`3.12.14`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| dipcatcher_total_fees | 131846.29515885914 |
| fills_dipcatcher | 1587 |
| <redacted>_max_abs_diff | 3.4924596548080444e-09 |
| <redacted>_max_rel_diff | 2.6329120621212285e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 1587 |
| vectorbt_total_fees | 131846.2951588591 |

| latency field | value |
| --- | --- |
| reps | 10 |
| dipcatcher_ms_median | 52.60380000981968 |
| vectorbt_ms_median | 68.25610001396853 |
| dipcatcher_ms_all | 52.49, 56.5, 45.09, 48.73, 39.71, 57.03, 55.52, 57.71, 52.71, 50.79 |
| vectorbt_ms_all | 68.31, 62.24, 69.4, 72.85, 72.25, 63.2, 62.48, 63.39, 68.2, 70.88 |

Workload fields copied from the receipt:

- `assets`:
  - BTCUSDT
  - ETHUSDT
  - SOLUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/3 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 1437
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: BTCUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1523565.55
  - `stale_marks`: accepted; equity NaN rows=0, final=1485050.3295675514

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-3a-numba.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-3a-numba.json`:

<!-- verbatim-receipt-text -->
> cash constraint: with weights summing to exactly 1.0, dipcatcher rejects whole orders that would overdraw cash (329 rejects on the 3-asset workload), while vectorbt clips/fills; workloads use a 0.9 target-sum buffer so the parity test isolates accounting.
<!-- /verbatim-receipt-text -->

### `.dsh-24x7/evidence-incumbent-vectorbt-fast-v2.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt-fast-v2.json | a85c1a06c91093a46aaad02e9e17cce79d3e47cf5c24378603a365252827f19d | absent | no_embedded_seal | absent | workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded engine: `fast`
Recorded environment: `platform`=`win32`, `python`=`3.12.14`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| dipcatcher_total_fees | 131846.29515885914 |
| fills_dipcatcher | 1587 |
| <redacted>_max_abs_diff | 3.4924596548080444e-09 |
| <redacted>_max_rel_diff | 2.6329120621212285e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 1587 |
| vectorbt_total_fees | 131846.2951588591 |

| latency field | value |
| --- | --- |
| reps | 10 |
| dipcatcher_ms_median | 639.8274999955902 |
| vectorbt_ms_median | 265.6039000139572 |
| dipcatcher_ms_all | 787.73, 690.6, 630.29, 649.36, 606.87, 801.2, 798.28, 613.83, 521.5, 514.56 |
| vectorbt_ms_all | 269.25, 240.7, 261.95, 258.15, 321.12, 322.91, 271.67, 235.15, 281.25, 245.58 |

Workload fields copied from the receipt:

- `assets`:
  - BTCUSDT
  - ETHUSDT
  - SOLUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/3 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 1437
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: BTCUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1523565.55
  - `stale_marks`: accepted; equity NaN rows=0, final=1485050.3295675514

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-v2.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast-v2.json`:

<!-- verbatim-receipt-text -->
> cash constraint: with weights summing to exactly 1.0, dipcatcher rejects whole orders that would overdraw cash (329 rejects on the 3-asset workload), while vectorbt clips/fills; workloads use a 0.9 target-sum buffer so the parity test isolates accounting.
<!-- /verbatim-receipt-text -->

### `.dsh-24x7/evidence-incumbent-vectorbt-fast.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt-fast.json | 7045bd91650f0d064a9bf89451aac5f94a796cb4169029e1effb2af46d9332aa | absent | no_embedded_seal | absent | workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded engine: `fast`
Recorded environment: `platform`=`win32`, `python`=`3.12.14`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| dipcatcher_total_fees | 131846.29515885914 |
| fills_dipcatcher | 1587 |
| <redacted>_max_abs_diff | 3.4924596548080444e-09 |
| <redacted>_max_rel_diff | 2.6329120621212285e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 1587 |
| vectorbt_total_fees | 131846.2951588591 |

| latency field | value |
| --- | --- |
| reps | 10 |
| dipcatcher_ms_median | 304.7668499930296 |
| vectorbt_ms_median | 178.96999999356922 |
| dipcatcher_ms_all | 342.7, 315.85, 330.14, 286.91, 320.85, 280.92, 293.68, 341.53, 274.81, 292.42 |
| vectorbt_ms_all | 159.5, 186.2, 196.3, 176.07, 181.87, 154.9, 169.9, 206.95, 161.26, 187.86 |

Workload fields copied from the receipt:

- `assets`:
  - BTCUSDT
  - ETHUSDT
  - SOLUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/3 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 1437
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: BTCUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1523565.55
  - `stale_marks`: accepted; equity NaN rows=0, final=1485050.3295675514

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-fast.json`:

<!-- verbatim-receipt-text -->
> cash constraint: with weights summing to exactly 1.0, dipcatcher rejects whole orders that would overdraw cash (329 rejects on the 3-asset workload), while vectorbt clips/fills; workloads use a 0.9 target-sum buffer so the parity test isolates accounting.
<!-- /verbatim-receipt-text -->

### `.dsh-24x7/evidence-incumbent-vectorbt-v1.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt-v1.json | c666214071e857e3caa22498f8855e1605cb1d5417cc5cdee1fb373261e71766 | absent | no_embedded_seal | absent | workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded environment: `platform`=`darwin`, `python`=`3.12.13`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| fills_dipcatcher | 1587 |
| <redacted>_max_abs_diff | 3.4924596548080444e-09 |
| <redacted>_max_rel_diff | 2.6329120621212285e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 1587 |
| vectorbt_total_fees | 131846.2951588591 |

| latency field | value |
| --- | --- |
| reps | 10 |
| dipcatcher_ms_median | 720.4113124998912 |
| vectorbt_ms_median | 17.251166501409898 |
| dipcatcher_ms_all | 987.42, 679.11, 672.61, 692.66, 693.85, 731.53, 786.83, 753.55, 714.26, 726.56 |
| vectorbt_ms_all | 21.34, 18.1, 17.46, 17.87, 17.51, 16.63, 17.04, 16.74, 16.75, 16.68 |

Workload fields copied from the receipt:

- `assets`:
  - BTCUSDT
  - ETHUSDT
  - SOLUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/3 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 1437
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: BTCUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1523565.55
  - `stale_marks`: accepted; equity NaN rows=0, final=1485050.3295675514

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-v1.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt-v1.json`:

Known semantic differences: absent

### `.dsh-24x7/evidence-incumbent-vectorbt.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .dsh-24x7/evidence-incumbent-vectorbt.json | 3985617cbac4667f6cb0019dee423079391bb38eec53a08b4dfdc875c7663fcc | absent | no_embedded_seal | absent | workload.bar_files_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, workload.bar_files_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, workload.bar_files_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb | real | absent | true | false |

Incumbent: `vectorbt` `1.1.0`
Schema: `incumbent_bench.v1`
Recorded environment: `platform`=`darwin`, `python`=`3.12.13`

| field | value |
| --- | --- |
| cash_rejects_dipcatcher | 0 |
| common_dates | 998 |
| dipcatcher_total_fees | 131846.29515885914 |
| fills_dipcatcher | 1587 |
| <redacted>_max_abs_diff | 3.4924596548080444e-09 |
| <redacted>_max_rel_diff | 2.6329120621212285e-15 |
| risk_gate_rejects_dipcatcher | 0 |
| vectorbt_orders | 1587 |
| vectorbt_total_fees | 131846.2951588591 |

| latency field | value |
| --- | --- |
| reps | 10 |
| dipcatcher_ms_median | 88.28868749969843 |
| vectorbt_ms_median | 49.58506250022765 |
| dipcatcher_ms_all | 89.03, 90.87, 98.42, 94.1, 87.54, 82.82, 78.7, 81.81, 85.33, 91.07 |
| vectorbt_ms_all | 46.36, 48.75, 46.76, 54.15, 59.51, 56.87, 53.98, 49.04, 48.0, 50.13 |

Workload fields copied from the receipt:

- `assets`:
  - BTCUSDT
  - ETHUSDT
  - SOLUSDT
- `bars_per_asset`: 999
- `commission_bps`: 10.0
- `spread_impact_borrow`: 0.0
- `strategy`: causal SMA20 gate, equal-weight 1/3 per gated name

Fault injection (forbidden-key fields omitted):

- `duplicate_weight_rows`: rejected: duplicate target weights for event_time/security_id
- `kill_switch_halts`:
  - `flat_equity`: true
  - `halt_rejects`: 1437
- `stale_held_marks`: rejected: held position valuation is stale beyond the configured limit: BTCUSDT=4
- `vectorbt_counterpart`:
  - `conflicting_target`: accepted, final=1523565.55
  - `stale_marks`: accepted; equity NaN rows=0, final=1485050.3295675514

Omitted 6 fields whose names tokenize to a forbidden research-headline metric.

Disclaimer copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt.json`:

<!-- verbatim-receipt-text -->
> Single matched workload vs vectorbt on real Binance daily bars. Latency is single-process wall time for identical economics; correctness is NAV parity; reliability demos are behavioural. Not a claim of superiority across all product dimensions.
<!-- /verbatim-receipt-text -->

Known semantic differences copied verbatim from `.dsh-24x7/evidence-incumbent-vectorbt.json`:

<!-- verbatim-receipt-text -->
> cash constraint: with weights summing to exactly 1.0, dipcatcher rejects whole orders that would overdraw cash (329 rejects on the 3-asset workload), while vectorbt clips/fills; workloads use a 0.9 target-sum buffer so the parity test isolates accounting.
<!-- /verbatim-receipt-text -->

## Crypto dip bench

Climatology baseline on the committed crypto dip-bench receipt.
Brier, ECE, and log loss are the receipt's proper scores.

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| receipts/dip_bench_crypto_1d_20260925.json | 584eb681dcd18fc65835c1573360b4b5fc9ebb72aa662bcbb9e7c101b06e2f03 | absent | no_embedded_seal | absent | inputs_sha256.adausdt_1d.parquet=59b5ec1a9076065bbedc3e14e49e59ee5f3781646721710dffca0aeab093a3f2, inputs_sha256.avaxusdt_1d.parquet=40028d2c07bced6cc2d37b6629538959f99fabc8b6653c8c696d87c0a7cc2359, inputs_sha256.bnbusdt_1d.parquet=822ec14bb30fda27bd14b4ab2a99070ae2d78858794c08444f8c0adbf75a80c1, inputs_sha256.btcusdt_1d.parquet=4467995a27dc580f8ace646bad30fe0c04f76dddd7454ba2822a057b8267e0c6, inputs_sha256.dogeusdt_1d.parquet=a8a8005ee958a265c313ac6ee3da9f0dacf1e1d7be95b7496bb5a58d687bf7c7, inputs_sha256.ethusdt_1d.parquet=6f560f88c070d2845e8da146e50963f4bd69b1dcef31cf4b481c5b5f9ce60250, inputs_sha256.linkusdt_1d.parquet=e09b987e4a7c9fbb9b8fe659d68677d75fe687b963451b09f14698dbe997b512, inputs_sha256.ltcusdt_1d.parquet=ce5bd0ab9540792895940afae9e0934db0eaf0e221bf3ae7bd3d1e89ce361b3a, inputs_sha256.solusdt_1d.parquet=bda7989c04cf8014a6aae32f92a23800a1b42e434108d4ff85b3192e24d393fb, inputs_sha256.trxusdt_1d.parquet=9eb8c62328057e0eef19013be9e435b585d880a9ebe14a6f59bc1cc14427d3f3, inputs_sha256.xrpusdt_1d.parquet=d03430a83a62632adaa02878069c6124f9855247238fc2f635d19b0c6bf91f30 | real | absent | true | false |

Recorded data label: `historical 1d bars (real market data; descriptive research)`
Events: `176`
Generated at: `2026-09-25T14:18:53.413467+00:00`

Parameters:

- `horizons_bars`:
  - `12m`: 252
  - `1m`: 21
  - `3m`: 63
  - `6m`: 126
- `threshold`: 0.1

| horizon | baseline_recovery |
| --- | --- |
| 12m | 0.7443181818181818 |
| 1m | 0.2159090909090909 |
| 3m | 0.5113636363636364 |
| 6m | 0.5511363636363636 |

Climatology definition:

P(recovery) = unconditional baseline frequency (in-sample, descriptive — not a signal)

| metric | value |
| --- | --- |
| brier_12m | 0.19030862603305784 |
| brier_1m | 0.16929235537190082 |
| brier_3m | 0.24987086776859505 |
| brier_6m | 0.24738507231404958 |
| brier_overall | 0.21421423037190082 |
| ece_12m | 0.0 |
| ece_1m | 0.0 |
| ece_3m | 0.0 |
| ece_6m | 0.0 |
| log_loss_12m | 0.5684916008392015 |
| log_loss_1m | 0.5216812528716921 |
| log_loss_3m | 0.6928888938590295 |
| log_loss_6m | 0.6879081696988175 |
| n_12m | 176.0 |
| n_1m | 176.0 |
| n_3m | 176.0 |
| n_6m | 176.0 |

| asset | bars | events |
| --- | --- | --- |
| ADAUSDT | 999 | 9 |
| AVAXUSDT | 999 | 9 |
| BNBUSDT | 999 | 21 |
| BTCUSDT | 999 | 25 |
| DOGEUSDT | 999 | 12 |
| ETHUSDT | 999 | 17 |
| LINKUSDT | 999 | 14 |
| LTCUSDT | 999 | 23 |
| SOLUSDT | 999 | 22 |
| TRXUSDT | 999 | 9 |
| XRPUSDT | 999 | 15 |

Disclaimer copied verbatim from `receipts/dip_bench_crypto_1d_20260925.json`:

<!-- verbatim-receipt-text -->
> Research/backtest evidence on historical data, not live performance. The climatology forecaster is an in-sample descriptive baseline; it is not a tradeable signal and authorizes nothing.
<!-- /verbatim-receipt-text -->

## Other committed receipts

Remaining files under `receipts/`. Result fields are copied. Fields whose
names tokenize to a forbidden research-headline metric are omitted and
counted. Prose that contains such a token is quoted verbatim.

### `receipts/adaptive_mix_20asset_1d_20260922.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| receipts/adaptive_mix_20asset_1d_20260922.json | b59422415e556f674ea03fb8e9a3b867408bf6d163cf7c11c64feeb7e33b4313 | absent | no_embedded_seal | absent | input_hashes.adausdt.funding.parquet=3121dea41a3d971b1dff9e16d3dd07ba7f7731306d1b451b7eb4edf2d6778dab, input_hashes.adausdt_1d.perp.parquet=f06a3b4c084985bc5d4166393eb60a90ba829243eb6ad450486ea5360dc92951, input_hashes.aptusdt.funding.parquet=2fccda54f91815de3852038e6b68988a5d5a750f016edfc8360fd0808a0d5d53, input_hashes.aptusdt_1d.perp.parquet=c5ec8e6badd3de2097fc91a1f00198a1ccdeec77c971f1100a80453b75d7240d, input_hashes.arbusdt.funding.parquet=9d91771028d18cd5820db949fbd52cb83c5bae19c09d5823a6acb6a5ef9eec8c, input_hashes.arbusdt_1d.perp.parquet=349e879fa54f8569cfa2efccf96bcf47f7359fe7f1f8a3188380afdaa9c989a1, input_hashes.atomusdt.funding.parquet=b0aa0ddc3a6cc940b0c9149b82c27d61121334eef578af29778bb55d14edd87f, input_hashes.atomusdt_1d.perp.parquet=13d177c80cc6e65cf4c2f3a6831a17415a7083771ac18cc3e473b85429434760, input_hashes.avaxusdt.funding.parquet=8b2243986a3bdd5d1fe7362d41c1a984b44e25f2a709b3dead0376ad8cfcfa62, input_hashes.avaxusdt_1d.perp.parquet=31f164333caa19d70c207b622d4eda172c1ee1ff029ca4a7cb88c803a36dcc93, input_hashes.bchusdt.funding.parquet=b8c170ec7fa4a93b7b04b41de90a1a1908e83dc2eabd34e7f393ebc3b309c3bf, input_hashes.bchusdt_1d.perp.parquet=ae7fdedfc50baa70437a66106125e42aed7b8450a3f62876655d8c34e295cfc5, input_hashes.bnbusdt.funding.parquet=aa72fc43a9be3964527949c11bb1534234ff8b921d78622950f19641e9c01ff2, input_hashes.bnbusdt_1d.perp.parquet=d73c419c08cee6fcaf5e6bd605a8bbefa7543acf1c4aae0a234a5146ceda20fa, input_hashes.btcusdt.funding.parquet=88869e924cb5117c7b192846172944201f1c68edc18cdd70510cbd82d3f63fbb, input_hashes.btcusdt_1d.perp.parquet=40dc02291c10fcdc459fdda31eafd649d894793a5641518157d886ef25ca541a, input_hashes.dogeusdt.funding.parquet=cbbcd14947a99abbe006a7b948b628f526def777092b03fabbec5f2dd833c5df, input_hashes.dogeusdt_1d.perp.parquet=d96509d1c25207c9b4bafe48fdd14c0c8363c9d22ba44e4712506b55e33adc45, input_hashes.dotusdt.funding.parquet=5c7e24aad965dd4cfb045430b74b9458749f437b8b622ebfe3e495342c1da4b5, input_hashes.dotusdt_1d.perp.parquet=a2bdb79938cfe010e55ef9ff8a10f6275be0a7578ce1a2412fe52dd22d4959e4, input_hashes.ethusdt.funding.parquet=4ade6cddbad88d2947856a5b01faa37236272be6f8e94bd1c8c29f2427d612d2, input_hashes.ethusdt_1d.perp.parquet=5e18e263c57d3cc08dca49e09229b469dc58b2d9b916b357306f755ac6415ecb, input_hashes.filusdt.funding.parquet=0259dd625800dc873c25c318cfd1eeada8d48f5a85a7fc0adb1a961ea29a9e93, input_hashes.filusdt_1d.perp.parquet=5e01b97a5dab2fb7e5fac0b3b8b2169860001b20e344d6ca833926b91476f330, input_hashes.linkusdt.funding.parquet=bb67718195e20e2ffc25c78f2fb520ebeedbca1761d19d8e7d4d464d48445b98, input_hashes.linkusdt_1d.perp.parquet=5957081c3773ca644cc5b62d02f1fef36dd52d4eb74ec45c2568f6badaf3e033, input_hashes.ltcusdt.funding.parquet=90bbb126ed3941b4a23534650f91d9184146d0db2be59e1c41c00501f2d7681c, input_hashes.ltcusdt_1d.perp.parquet=58f778841bfd806499bda2349bdf955082cb1bfd417769e5620288f55640f039, input_hashes.nearusdt.funding.parquet=6ea882a1e14d65dcbee88a6c046166dea347b63838ba818697816d48600a54a0, input_hashes.nearusdt_1d.perp.parquet=5a564fa008ed7cb66e75290de1a55d0b5da5d44ff34d6b744a189ef2be9eebed, input_hashes.solusdt.funding.parquet=7675713eeb78c9beb362ec29fbb77f528f5c2489d0b61221e7b57a7b2393ef05, input_hashes.solusdt_1d.perp.parquet=20920ce122dc212cc495e4ba8a36b64e251688aa0ebc2a3922dffaa219bf2ba4, input_hashes.suiusdt.funding.parquet=83f972ffd68bc1db53a610a3a9c19ea89d20821519054976ea31ee1e00dae10e, input_hashes.suiusdt_1d.perp.parquet=f3ea5a801b0379501c29c0d994e0fa8eba5c7b32824b9de19d0ced4e91a90fab, input_hashes.trxusdt.funding.parquet=80ffb4b6e52fb1be033ce9d7984d9d2270962856ca54ce5d64ee8b11db895d1c, input_hashes.trxusdt_1d.perp.parquet=b12ff45a4d31f321acd19890c33da494f2850dde9fdd3b5414a6fd8ffd5f61e5, input_hashes.uniusdt.funding.parquet=0c6c104dde6a2a39a1153818abd50243f9675397125b59249cf087d322901254, input_hashes.uniusdt_1d.perp.parquet=67f4d8ede2aaa087d805a898e3e3429e9988dc571f60ffe8b698310ec3dc2801, input_hashes.xrpusdt.funding.parquet=a1465a46d13fe61d7f2b47fdc4ce3b84910a67ae18ef5fedfee86dad54491fdd, input_hashes.xrpusdt_1d.perp.parquet=528661bd82bb8e7828cd60cd6dec3d3832ad0499b000690ce358dd1c968746c3 | unspecified | absent | true | false |

- `schema`: adaptive_mix_replay.v1
- `created_at`: 2026-09-22T15:44:07.498331+00:00
- `evidence_level`: historical_exploratory_tail_reused
- `research_only`: true
- `data_source`: binance_usdtm_perp
- `n_assets`: 20
- `bar_count`: 43912
- `holdout_cut`: 2025-04-26 00:00:00+00:00
- `allocator`:
  - `window`: 60
  - `min_obs`: 20
  - `temperature`: 1.0
  - `equal_anchor`: 0.2
  - `input`:
<!-- verbatim-receipt-text -->
> costed sleeve NAV through completed close
<!-- /verbatim-receipt-text -->
  - `fill`: following bar open
- `results`:
  - `adaptive`:
    - `segments`:
      - `development`:
        - `n`: 2055
        - `net_return`: -0.050257767747253546
        - `cagr`: -0.009123090410010026
        - `max_drawdown`: -0.08711026995149562
      - `historical_tail_exploratory`:
        - `n`: 514
        - `net_return`: 0.02571011661273359
        - `cagr`: 0.018202461927767866
        - `max_drawdown`: -0.029565209645673107
    - `full_path_risk_gate_rejects`: 68
    - `full_path_liquidations`: 0
    - `full_path_costs`:
      - `commission`: 10707.819063275427
      - `spread`: 53539.095316377076
      - `impact`: 2931.9030167522837
  - `equal`:
    - `segments`:
      - `development`:
        - `n`: 2055
        - `net_return`: -0.09162984419899656
        - `cagr`: -0.0169360824848912
        - `max_drawdown`: -0.13144875185724092
      - `historical_tail_exploratory`:
        - `n`: 514
        - `net_return`: 0.04438495980724566
        - `cagr`: 0.031341295283180415
        - `max_drawdown`: -0.03316082801239051
    - `full_path_risk_gate_rejects`: 42
    - `full_path_liquidations`: 0
    - `full_path_costs`:
      - `commission`: 9199.821275233466
      - `spread`: 45999.10637616721
      - `impact`: 2661.070875599598
- `paper_sleeves`:
  - `carry`:
    - `development`:
      - `n`: 2055
      - `net_return`: 0.13921179068155332
      - `cagr`: 0.023436075933308764
      - `max_drawdown`: -0.02124751383521928
    - `historical_tail_exploratory`:
      - `n`: 514
      - `net_return`: -0.01292817978287042
      - `cagr`: -0.009204086642884413
      - `max_drawdown`: -0.01954388780574723
  - `fade`:
    - `development`:
      - `n`: 2055
      - `net_return`: -0.1010058568098029
      - `cagr`: -0.018747281400395388
      - `max_drawdown`: -0.12347201732005986
    - `historical_tail_exploratory`:
      - `n`: 514
      - `net_return`: 0.014295825918295924
      - `cagr`: 0.010137772432946779
      - `max_drawdown`: -0.015142390623816837
  - `momentum`:
    - `development`:
      - `n`: 2055
      - `net_return`: 0.1829170111890468
      - `cagr`: 0.030307095892233793
      - `max_drawdown`: -0.19683287994774645
    - `historical_tail_exploratory`:
      - `n`: 514
      - `net_return`: 0.08068636345502589
      - `cagr`: 0.05668876861045957
      - `max_drawdown`: -0.05448879753982827
  - `sweep`:
    - `development`:
      - `n`: 2055
      - `net_return`: -0.45771614875694644
      - `cagr`: -0.10306249168386028
      - `max_drawdown`: -0.4901462881448232
    - `historical_tail_exploratory`:
      - `n`: 514
      - `net_return`: 0.04046353851169804
      - `cagr`: 0.028588026058157467
      - `max_drawdown`: -0.06868424667379747
  - `trend`:
    - `development`:
      - `n`: 2055
      - `net_return`: 0.03611793221429993
      - `cagr`: 0.00632621597641192
      - `max_drawdown`: -0.12078424009757494
    - `historical_tail_exploratory`:
      - `n`: 514
      - `net_return`: 0.058717960871798924
      - `cagr`: 0.04137930948008739
      - `max_drawdown`: -0.07982618739838132
- `mean_adaptive_allocation`:
  - `development`:
    - `carry`: 0.2484398409058769
    - `fade`: 0.1665964980707751
    - `momentum`: 0.22625875949232244
    - `sweep`: 0.16271173590126117
    - `trend`: 0.19599316562976446
  - `historical_tail_exploratory`:
    - `carry`: 0.18021988979794917
    - `fade`: 0.1835988639194764
    - `momentum`: 0.2162850206060653
    - `sweep`: 0.24675865974495423
    - `trend`: 0.17313756593155488

Omitted 14 fields whose names tokenize to a forbidden research-headline metric.

### `receipts/adaptive_mix_band_search_20asset_1d_20260922.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| receipts/adaptive_mix_band_search_20asset_1d_20260922.json | 5e02918850944a68d1d45815b5928f1eed462d456eac4723ac9290514e7b97f4 | absent | no_embedded_seal | absent | input_hashes.adausdt.funding.parquet=3121dea41a3d971b1dff9e16d3dd07ba7f7731306d1b451b7eb4edf2d6778dab, input_hashes.adausdt_1d.perp.parquet=f06a3b4c084985bc5d4166393eb60a90ba829243eb6ad450486ea5360dc92951, input_hashes.aptusdt.funding.parquet=2fccda54f91815de3852038e6b68988a5d5a750f016edfc8360fd0808a0d5d53, input_hashes.aptusdt_1d.perp.parquet=c5ec8e6badd3de2097fc91a1f00198a1ccdeec77c971f1100a80453b75d7240d, input_hashes.arbusdt.funding.parquet=9d91771028d18cd5820db949fbd52cb83c5bae19c09d5823a6acb6a5ef9eec8c, input_hashes.arbusdt_1d.perp.parquet=349e879fa54f8569cfa2efccf96bcf47f7359fe7f1f8a3188380afdaa9c989a1, input_hashes.atomusdt.funding.parquet=b0aa0ddc3a6cc940b0c9149b82c27d61121334eef578af29778bb55d14edd87f, input_hashes.atomusdt_1d.perp.parquet=13d177c80cc6e65cf4c2f3a6831a17415a7083771ac18cc3e473b85429434760, input_hashes.avaxusdt.funding.parquet=8b2243986a3bdd5d1fe7362d41c1a984b44e25f2a709b3dead0376ad8cfcfa62, input_hashes.avaxusdt_1d.perp.parquet=31f164333caa19d70c207b622d4eda172c1ee1ff029ca4a7cb88c803a36dcc93, input_hashes.bchusdt.funding.parquet=b8c170ec7fa4a93b7b04b41de90a1a1908e83dc2eabd34e7f393ebc3b309c3bf, input_hashes.bchusdt_1d.perp.parquet=ae7fdedfc50baa70437a66106125e42aed7b8450a3f62876655d8c34e295cfc5, input_hashes.bnbusdt.funding.parquet=aa72fc43a9be3964527949c11bb1534234ff8b921d78622950f19641e9c01ff2, input_hashes.bnbusdt_1d.perp.parquet=d73c419c08cee6fcaf5e6bd605a8bbefa7543acf1c4aae0a234a5146ceda20fa, input_hashes.btcusdt.funding.parquet=88869e924cb5117c7b192846172944201f1c68edc18cdd70510cbd82d3f63fbb, input_hashes.btcusdt_1d.perp.parquet=40dc02291c10fcdc459fdda31eafd649d894793a5641518157d886ef25ca541a, input_hashes.dogeusdt.funding.parquet=cbbcd14947a99abbe006a7b948b628f526def777092b03fabbec5f2dd833c5df, input_hashes.dogeusdt_1d.perp.parquet=d96509d1c25207c9b4bafe48fdd14c0c8363c9d22ba44e4712506b55e33adc45, input_hashes.dotusdt.funding.parquet=5c7e24aad965dd4cfb045430b74b9458749f437b8b622ebfe3e495342c1da4b5, input_hashes.dotusdt_1d.perp.parquet=a2bdb79938cfe010e55ef9ff8a10f6275be0a7578ce1a2412fe52dd22d4959e4, input_hashes.ethusdt.funding.parquet=4ade6cddbad88d2947856a5b01faa37236272be6f8e94bd1c8c29f2427d612d2, input_hashes.ethusdt_1d.perp.parquet=5e18e263c57d3cc08dca49e09229b469dc58b2d9b916b357306f755ac6415ecb, input_hashes.filusdt.funding.parquet=0259dd625800dc873c25c318cfd1eeada8d48f5a85a7fc0adb1a961ea29a9e93, input_hashes.filusdt_1d.perp.parquet=5e01b97a5dab2fb7e5fac0b3b8b2169860001b20e344d6ca833926b91476f330, input_hashes.linkusdt.funding.parquet=bb67718195e20e2ffc25c78f2fb520ebeedbca1761d19d8e7d4d464d48445b98, input_hashes.linkusdt_1d.perp.parquet=5957081c3773ca644cc5b62d02f1fef36dd52d4eb74ec45c2568f6badaf3e033, input_hashes.ltcusdt.funding.parquet=90bbb126ed3941b4a23534650f91d9184146d0db2be59e1c41c00501f2d7681c, input_hashes.ltcusdt_1d.perp.parquet=58f778841bfd806499bda2349bdf955082cb1bfd417769e5620288f55640f039, input_hashes.nearusdt.funding.parquet=6ea882a1e14d65dcbee88a6c046166dea347b63838ba818697816d48600a54a0, input_hashes.nearusdt_1d.perp.parquet=5a564fa008ed7cb66e75290de1a55d0b5da5d44ff34d6b744a189ef2be9eebed, input_hashes.solusdt.funding.parquet=7675713eeb78c9beb362ec29fbb77f528f5c2489d0b61221e7b57a7b2393ef05, input_hashes.solusdt_1d.perp.parquet=20920ce122dc212cc495e4ba8a36b64e251688aa0ebc2a3922dffaa219bf2ba4, input_hashes.suiusdt.funding.parquet=83f972ffd68bc1db53a610a3a9c19ea89d20821519054976ea31ee1e00dae10e, input_hashes.suiusdt_1d.perp.parquet=f3ea5a801b0379501c29c0d994e0fa8eba5c7b32824b9de19d0ced4e91a90fab, input_hashes.trxusdt.funding.parquet=80ffb4b6e52fb1be033ce9d7984d9d2270962856ca54ce5d64ee8b11db895d1c, input_hashes.trxusdt_1d.perp.parquet=b12ff45a4d31f321acd19890c33da494f2850dde9fdd3b5414a6fd8ffd5f61e5, input_hashes.uniusdt.funding.parquet=0c6c104dde6a2a39a1153818abd50243f9675397125b59249cf087d322901254, input_hashes.uniusdt_1d.perp.parquet=67f4d8ede2aaa087d805a898e3e3429e9988dc571f60ffe8b698310ec3dc2801, input_hashes.xrpusdt.funding.parquet=a1465a46d13fe61d7f2b47fdc4ce3b84910a67ae18ef5fedfee86dad54491fdd, input_hashes.xrpusdt_1d.perp.parquet=528661bd82bb8e7828cd60cd6dec3d3832ad0499b000690ce358dd1c968746c3 | unspecified | absent | true | false |

- `schema`: adaptive_mix_band_search.v1
- `created_at`: 2026-09-22T15:44:08.787931+00:00
- `evidence_level`: historical_development_only
- `research_only`: true
- `n_assets`: 20
- `holdout_cut`: 2025-04-26 00:00:00+00:00
- `selection_rule`:
<!-- verbatim-receipt-text -->
> positive net CAGR and Sharpe, max drawdown under 5%; then highest development Sharpe
<!-- /verbatim-receipt-text -->
- `candidates`:
  - [0]
    - `band`: 0.0
    - `development`:
      - `n`: 2055
      - `net_return`: -0.050257767747253546
      - `cagr`: -0.009123090410010026
      - `max_drawdown`: -0.08711026995149562
    - `eligible`: false
    - `n_target_requests`: 33632
    - `risk_gate_rejects`: 37
    - `costs`:
      - `commission`: 8224.208607185903
      - `spread`: 41121.04303592964
      - `impact`: 2304.530065197563
  - [1]
    - `band`: 0.0025
    - `development`:
      - `n`: 2055
      - `net_return`: -0.09216156577727797
      - `cagr`: -0.01703838495591936
      - `max_drawdown`: -0.10523395988832385
    - `eligible`: false
    - `n_target_requests`: 9551
    - `risk_gate_rejects`: 23
    - `costs`:
      - `commission`: 6438.732052437595
      - `spread`: 32193.66026218792
      - `impact`: 2072.5069912422623
  - [2]
    - `band`: 0.005
    - `development`:
      - `n`: 2055
      - `net_return`: -0.07140452984843793
      - `cagr`: -0.013080836079716662
      - `max_drawdown`: -0.09359214429608098
    - `eligible`: false
    - `n_target_requests`: 5592
    - `risk_gate_rejects`: 28
    - `costs`:
      - `commission`: 5460.666130636665
      - `spread`: 27303.33065318333
      - `impact`: 1956.104097916909
  - [3]
    - `band`: 0.01
    - `development`:
      - `n`: 2055
      - `net_return`: -0.019231316573260715
      - `cagr`: -0.0034454666210986806
      - `max_drawdown`: -0.1139716110675979
    - `eligible`: false
    - `n_target_requests`: 2518
    - `risk_gate_rejects`: 13
    - `costs`:
      - `commission`: 3813.098350123
      - `spread`: 19065.491750614976
      - `impact`: 1629.4556694918513
  - [4]
    - `band`: 0.02
    - `development`:
      - `n`: 2055
      - `net_return`: -0.13106407203519255
      - `cagr`: -0.024660410714203884
      - `max_drawdown`: -0.27657440301956515
    - `eligible`: false
    - `n_target_requests`: 662
    - `risk_gate_rejects`: 3
    - `costs`:
      - `commission`: 1433.8767650540135
      - `spread`: 7169.383825270064
      - `impact`: 826.4444476240769
- `selected_band`: null

Omitted 5 fields whose names tokenize to a forbidden research-headline metric.

### `receipts/basis_pair_candidate_20asset_1d_20260922.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| receipts/basis_pair_candidate_20asset_1d_20260922.json | d021801c94e33d8720f9084762138893978a61e36ef670b4c0140edeaf37c1ec | absent | no_embedded_seal | absent | input_hashes.adausdt.funding.parquet=3121dea41a3d971b1dff9e16d3dd07ba7f7731306d1b451b7eb4edf2d6778dab, input_hashes.adausdt_1d.perp.parquet=f06a3b4c084985bc5d4166393eb60a90ba829243eb6ad450486ea5360dc92951, input_hashes.adausdt_1d.spot.parquet=8f6393c9c6056a922eced1612696e45a9c39ece856dee82324b665891e9b8304, input_hashes.aptusdt.funding.parquet=2fccda54f91815de3852038e6b68988a5d5a750f016edfc8360fd0808a0d5d53, input_hashes.aptusdt_1d.perp.parquet=c5ec8e6badd3de2097fc91a1f00198a1ccdeec77c971f1100a80453b75d7240d, input_hashes.aptusdt_1d.spot.parquet=0162c78cb3c1d64cad82ff53be6bd212af1dd40b06dd8e88fe5fd23f8501aa78, input_hashes.arbusdt.funding.parquet=9d91771028d18cd5820db949fbd52cb83c5bae19c09d5823a6acb6a5ef9eec8c, input_hashes.arbusdt_1d.perp.parquet=349e879fa54f8569cfa2efccf96bcf47f7359fe7f1f8a3188380afdaa9c989a1, input_hashes.arbusdt_1d.spot.parquet=14dd4758db2369d61f2a57696e2ceaef2aab8d703d91269a6971dad67f99ee98, input_hashes.atomusdt.funding.parquet=b0aa0ddc3a6cc940b0c9149b82c27d61121334eef578af29778bb55d14edd87f, input_hashes.atomusdt_1d.perp.parquet=13d177c80cc6e65cf4c2f3a6831a17415a7083771ac18cc3e473b85429434760, input_hashes.atomusdt_1d.spot.parquet=335499224afcc280ee3f0e25041652dafe3afe2b18e256a492c258adfe8d189a, input_hashes.avaxusdt.funding.parquet=8b2243986a3bdd5d1fe7362d41c1a984b44e25f2a709b3dead0376ad8cfcfa62, input_hashes.avaxusdt_1d.perp.parquet=31f164333caa19d70c207b622d4eda172c1ee1ff029ca4a7cb88c803a36dcc93, input_hashes.avaxusdt_1d.spot.parquet=07dc4a61d7d3c3add63f74b7f5d849797bd1d5e902a2e4ba4f0117fdc9b40ff4, input_hashes.bchusdt.funding.parquet=b8c170ec7fa4a93b7b04b41de90a1a1908e83dc2eabd34e7f393ebc3b309c3bf, input_hashes.bchusdt_1d.perp.parquet=ae7fdedfc50baa70437a66106125e42aed7b8450a3f62876655d8c34e295cfc5, input_hashes.bchusdt_1d.spot.parquet=81c05b927aa6afa0909f02eb52adfd97da79ca921ce3b3983859d5d2039814f5, input_hashes.bnbusdt.funding.parquet=aa72fc43a9be3964527949c11bb1534234ff8b921d78622950f19641e9c01ff2, input_hashes.bnbusdt_1d.perp.parquet=d73c419c08cee6fcaf5e6bd605a8bbefa7543acf1c4aae0a234a5146ceda20fa, input_hashes.bnbusdt_1d.spot.parquet=b42d2456a72867745bf663176c094850f4991975cbc72165fc5bf10f67fa1ccb, input_hashes.btcusdt.funding.parquet=88869e924cb5117c7b192846172944201f1c68edc18cdd70510cbd82d3f63fbb, input_hashes.btcusdt_1d.perp.parquet=40dc02291c10fcdc459fdda31eafd649d894793a5641518157d886ef25ca541a, input_hashes.btcusdt_1d.spot.parquet=3dbd53cd2a483f37dac9599786f2dadbca5c122734dc63d59e8dc606897d5c22, input_hashes.dogeusdt.funding.parquet=cbbcd14947a99abbe006a7b948b628f526def777092b03fabbec5f2dd833c5df, input_hashes.dogeusdt_1d.perp.parquet=d96509d1c25207c9b4bafe48fdd14c0c8363c9d22ba44e4712506b55e33adc45, input_hashes.dogeusdt_1d.spot.parquet=dc2589d4d64fea57bd65a45d699392463a0d1f9da644d97b3bee4c4968fde88f, input_hashes.dotusdt.funding.parquet=5c7e24aad965dd4cfb045430b74b9458749f437b8b622ebfe3e495342c1da4b5, input_hashes.dotusdt_1d.perp.parquet=a2bdb79938cfe010e55ef9ff8a10f6275be0a7578ce1a2412fe52dd22d4959e4, input_hashes.dotusdt_1d.spot.parquet=d31160f702dea385addfe50a4de9794dfae26a892ad738de1eb4ea011040315f, input_hashes.ethusdt.funding.parquet=4ade6cddbad88d2947856a5b01faa37236272be6f8e94bd1c8c29f2427d612d2, input_hashes.ethusdt_1d.perp.parquet=5e18e263c57d3cc08dca49e09229b469dc58b2d9b916b357306f755ac6415ecb, input_hashes.ethusdt_1d.spot.parquet=daebae805af104c5f9de6f26e4eeafe64394ae71d393e2b9c0c1169d4cf184c5, input_hashes.filusdt.funding.parquet=0259dd625800dc873c25c318cfd1eeada8d48f5a85a7fc0adb1a961ea29a9e93, input_hashes.filusdt_1d.perp.parquet=5e01b97a5dab2fb7e5fac0b3b8b2169860001b20e344d6ca833926b91476f330, input_hashes.filusdt_1d.spot.parquet=d116cb9e54f168067ac2f58b0e2c02c445a0c0765781f95f72a62bab8346bebb, input_hashes.linkusdt.funding.parquet=bb67718195e20e2ffc25c78f2fb520ebeedbca1761d19d8e7d4d464d48445b98, input_hashes.linkusdt_1d.perp.parquet=5957081c3773ca644cc5b62d02f1fef36dd52d4eb74ec45c2568f6badaf3e033, input_hashes.linkusdt_1d.spot.parquet=4e9dfb100be0b659893e9a99737625eef4f27f2c40a8e49cb22d03f051470819, input_hashes.ltcusdt.funding.parquet=90bbb126ed3941b4a23534650f91d9184146d0db2be59e1c41c00501f2d7681c, input_hashes.ltcusdt_1d.perp.parquet=58f778841bfd806499bda2349bdf955082cb1bfd417769e5620288f55640f039, input_hashes.ltcusdt_1d.spot.parquet=7106b554434e8c887a736efc01501793d4377014ad729f7c3308fde450fa8a14, input_hashes.nearusdt.funding.parquet=6ea882a1e14d65dcbee88a6c046166dea347b63838ba818697816d48600a54a0, input_hashes.nearusdt_1d.perp.parquet=5a564fa008ed7cb66e75290de1a55d0b5da5d44ff34d6b744a189ef2be9eebed, input_hashes.nearusdt_1d.spot.parquet=0f9454be77d4e3ae74befa7d0b308bc0c943e3b980afe07bb9fd356151dce8fb, input_hashes.solusdt.funding.parquet=7675713eeb78c9beb362ec29fbb77f528f5c2489d0b61221e7b57a7b2393ef05, input_hashes.solusdt_1d.perp.parquet=20920ce122dc212cc495e4ba8a36b64e251688aa0ebc2a3922dffaa219bf2ba4, input_hashes.solusdt_1d.spot.parquet=68f14a781c0243037bf0397726543a8e23d794862862c59d06c2594be97a59d6, input_hashes.suiusdt.funding.parquet=83f972ffd68bc1db53a610a3a9c19ea89d20821519054976ea31ee1e00dae10e, input_hashes.suiusdt_1d.perp.parquet=f3ea5a801b0379501c29c0d994e0fa8eba5c7b32824b9de19d0ced4e91a90fab, input_hashes.suiusdt_1d.spot.parquet=b5c16d647d7465cdb4e5f3a4cd029b13aea7b34b4ab6fe0046a1ac256ae5d3e0, input_hashes.trxusdt.funding.parquet=80ffb4b6e52fb1be033ce9d7984d9d2270962856ca54ce5d64ee8b11db895d1c, input_hashes.trxusdt_1d.perp.parquet=b12ff45a4d31f321acd19890c33da494f2850dde9fdd3b5414a6fd8ffd5f61e5, input_hashes.trxusdt_1d.spot.parquet=ff7113fe2f0c8fafb27f0acdb0ede039625c23327e1de3a430c4332a5f25e84f, input_hashes.uniusdt.funding.parquet=0c6c104dde6a2a39a1153818abd50243f9675397125b59249cf087d322901254, input_hashes.uniusdt_1d.perp.parquet=67f4d8ede2aaa087d805a898e3e3429e9988dc571f60ffe8b698310ec3dc2801, input_hashes.uniusdt_1d.spot.parquet=42dbf32e89f03ea36d38bfdf5e4f98dec630baed501c3648cc3fe7b551ff483c, input_hashes.xrpusdt.funding.parquet=a1465a46d13fe61d7f2b47fdc4ce3b84910a67ae18ef5fedfee86dad54491fdd, input_hashes.xrpusdt_1d.perp.parquet=528661bd82bb8e7828cd60cd6dec3d3832ad0499b000690ce358dd1c968746c3, input_hashes.xrpusdt_1d.spot.parquet=c341c55f9aa945bcd110d7536fc26466f598d6534ef77b8dc955903c4960bf2d | unspecified | absent | true | false |

- `schema`: basis_pair_candidate.v1
- `created_at`: 2026-09-22T15:43:37.776452+00:00
- `evidence_level`: historical_development_only
- `research_only`: true
- `source`: binance_spot_perp_funding
- `survivorship`: currently listed Binance symbols; historical delistings absent
- `n_assets`: 20
- `development_cut`: 2025-04-26 00:00:00+00:00
- `n_target_events`: 4082
- `development`:
  - `total_return`: -0.09214520216663091
  - `cagr`: -0.017035235885943356
  - `max_drawdown`: -0.09216765565151375
  - `n`: 2055
  - `periods_per_year`: 365.25
  - `funding_net`: 27118.42200570218
  - `mean_turnover`: 0.05794611672643284
  - `liquidation_count`: 0
  - `risk_gate_rejects`: 561
  - `commission`: 22763.080253655593
  - `spread`: 113815.40126827796
  - `impact`: 26287.22382882894
  - `ruined`: false
- `development_eligible`: false

Omitted 3 fields whose names tokenize to a forbidden research-headline metric.

### `receipts/basis_reversion_screen_20asset_1d_20260922.json`

| source | file sha256 | embedded seal | seal | git revision | dataset hash | data class | promote | research_only | live_pnl_claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| receipts/basis_reversion_screen_20asset_1d_20260922.json | 61c9c8f5b3e502f0c5218dd743ad030ecbb962d6822bb7b81a2b962445aa378f | absent | no_embedded_seal | absent | input_hashes.adausdt_1d.perp.parquet=f06a3b4c084985bc5d4166393eb60a90ba829243eb6ad450486ea5360dc92951, input_hashes.adausdt_1d.spot.parquet=8f6393c9c6056a922eced1612696e45a9c39ece856dee82324b665891e9b8304, input_hashes.aptusdt_1d.perp.parquet=c5ec8e6badd3de2097fc91a1f00198a1ccdeec77c971f1100a80453b75d7240d, input_hashes.aptusdt_1d.spot.parquet=0162c78cb3c1d64cad82ff53be6bd212af1dd40b06dd8e88fe5fd23f8501aa78, input_hashes.arbusdt_1d.perp.parquet=349e879fa54f8569cfa2efccf96bcf47f7359fe7f1f8a3188380afdaa9c989a1, input_hashes.arbusdt_1d.spot.parquet=14dd4758db2369d61f2a57696e2ceaef2aab8d703d91269a6971dad67f99ee98, input_hashes.atomusdt_1d.perp.parquet=13d177c80cc6e65cf4c2f3a6831a17415a7083771ac18cc3e473b85429434760, input_hashes.atomusdt_1d.spot.parquet=335499224afcc280ee3f0e25041652dafe3afe2b18e256a492c258adfe8d189a, input_hashes.avaxusdt_1d.perp.parquet=31f164333caa19d70c207b622d4eda172c1ee1ff029ca4a7cb88c803a36dcc93, input_hashes.avaxusdt_1d.spot.parquet=07dc4a61d7d3c3add63f74b7f5d849797bd1d5e902a2e4ba4f0117fdc9b40ff4, input_hashes.bchusdt_1d.perp.parquet=ae7fdedfc50baa70437a66106125e42aed7b8450a3f62876655d8c34e295cfc5, input_hashes.bchusdt_1d.spot.parquet=81c05b927aa6afa0909f02eb52adfd97da79ca921ce3b3983859d5d2039814f5, input_hashes.bnbusdt_1d.perp.parquet=d73c419c08cee6fcaf5e6bd605a8bbefa7543acf1c4aae0a234a5146ceda20fa, input_hashes.bnbusdt_1d.spot.parquet=b42d2456a72867745bf663176c094850f4991975cbc72165fc5bf10f67fa1ccb, input_hashes.btcusdt_1d.perp.parquet=40dc02291c10fcdc459fdda31eafd649d894793a5641518157d886ef25ca541a, input_hashes.btcusdt_1d.spot.parquet=3dbd53cd2a483f37dac9599786f2dadbca5c122734dc63d59e8dc606897d5c22, input_hashes.dogeusdt_1d.perp.parquet=d96509d1c25207c9b4bafe48fdd14c0c8363c9d22ba44e4712506b55e33adc45, input_hashes.dogeusdt_1d.spot.parquet=dc2589d4d64fea57bd65a45d699392463a0d1f9da644d97b3bee4c4968fde88f, input_hashes.dotusdt_1d.perp.parquet=a2bdb79938cfe010e55ef9ff8a10f6275be0a7578ce1a2412fe52dd22d4959e4, input_hashes.dotusdt_1d.spot.parquet=d31160f702dea385addfe50a4de9794dfae26a892ad738de1eb4ea011040315f, input_hashes.ethusdt_1d.perp.parquet=5e18e263c57d3cc08dca49e09229b469dc58b2d9b916b357306f755ac6415ecb, input_hashes.ethusdt_1d.spot.parquet=daebae805af104c5f9de6f26e4eeafe64394ae71d393e2b9c0c1169d4cf184c5, input_hashes.filusdt_1d.perp.parquet=5e01b97a5dab2fb7e5fac0b3b8b2169860001b20e344d6ca833926b91476f330, input_hashes.filusdt_1d.spot.parquet=d116cb9e54f168067ac2f58b0e2c02c445a0c0765781f95f72a62bab8346bebb, input_hashes.linkusdt_1d.perp.parquet=5957081c3773ca644cc5b62d02f1fef36dd52d4eb74ec45c2568f6badaf3e033, input_hashes.linkusdt_1d.spot.parquet=4e9dfb100be0b659893e9a99737625eef4f27f2c40a8e49cb22d03f051470819, input_hashes.ltcusdt_1d.perp.parquet=58f778841bfd806499bda2349bdf955082cb1bfd417769e5620288f55640f039, input_hashes.ltcusdt_1d.spot.parquet=7106b554434e8c887a736efc01501793d4377014ad729f7c3308fde450fa8a14, input_hashes.nearusdt_1d.perp.parquet=5a564fa008ed7cb66e75290de1a55d0b5da5d44ff34d6b744a189ef2be9eebed, input_hashes.nearusdt_1d.spot.parquet=0f9454be77d4e3ae74befa7d0b308bc0c943e3b980afe07bb9fd356151dce8fb, input_hashes.solusdt_1d.perp.parquet=20920ce122dc212cc495e4ba8a36b64e251688aa0ebc2a3922dffaa219bf2ba4, input_hashes.solusdt_1d.spot.parquet=68f14a781c0243037bf0397726543a8e23d794862862c59d06c2594be97a59d6, input_hashes.suiusdt_1d.perp.parquet=f3ea5a801b0379501c29c0d994e0fa8eba5c7b32824b9de19d0ced4e91a90fab, input_hashes.suiusdt_1d.spot.parquet=b5c16d647d7465cdb4e5f3a4cd029b13aea7b34b4ab6fe0046a1ac256ae5d3e0, input_hashes.trxusdt_1d.perp.parquet=b12ff45a4d31f321acd19890c33da494f2850dde9fdd3b5414a6fd8ffd5f61e5, input_hashes.trxusdt_1d.spot.parquet=ff7113fe2f0c8fafb27f0acdb0ede039625c23327e1de3a430c4332a5f25e84f, input_hashes.uniusdt_1d.perp.parquet=67f4d8ede2aaa087d805a898e3e3429e9988dc571f60ffe8b698310ec3dc2801, input_hashes.uniusdt_1d.spot.parquet=42dbf32e89f03ea36d38bfdf5e4f98dec630baed501c3648cc3fe7b551ff483c, input_hashes.xrpusdt_1d.perp.parquet=528661bd82bb8e7828cd60cd6dec3d3832ad0499b000690ce358dd1c968746c3, input_hashes.xrpusdt_1d.spot.parquet=c341c55f9aa945bcd110d7536fc26466f598d6534ef77b8dc955903c4960bf2d | unspecified | absent | true | false |

- `schema`: basis_reversion_screen.v1
- `created_at`: 2026-09-22T15:37:20.535305+00:00
- `evidence_level`: development_pre_cost_diagnostic
- `research_only`: true
- `bar_interval`: 1d
- `signal`: negative 30-bar z-score of same-asset perp/spot closing basis
- `target`: next-bar perp open-to-close, long top quartile and short bottom quartile
- `development_cut`: 2025-04-26 00:00:00+00:00
- `n_assets`: 20
- `n_dates`: 1881
- `mean_daily_gross_spread`: 0.0015632331277941444
- `median_daily_gross_spread`: 0.0010410200759501664
- `positive_day_fraction`: 0.5231259968102073
- `four_chronological_fold_means`:
  - 0.005967220927493027
  - 0.00046005410140306937
  - 0.001252727212214577
  - -0.001436439916741964
- `hedged_pair_rule`: long spot/short perp when basis is positive and z-score exceeds 1
- `hedged_pair_excludes`: funding, commissions, spread, impact, margin costs
- `hedged_pair_active_date_fraction`: 0.5555555555555556
- `hedged_pair_mean_daily_gross_return`: 0.00018387810420158332
- `hedged_pair_four_fold_means`:
  - 0.00033789209517964727
  - 0.00015650157077502314
  - 0.00012397308872545248
  - 0.00011681797278370385
