...
class RobinhoodPlusBackend(str, Enum):
    """Research-only K-line backend choices."""
...
class RobinhoodPlusConfig(StrictConfigModel):
    """Causal, research-only Robinhood Plus challenger settings.

    The default configuration enables the deterministic NumPy challenger but
    assigns it zero blend weight, so it is measured and surfaced in diagnostics
    without sizing the production research book. Torch is local-only and must
    fail closed when weights are unavailable.
    """
    ...
    def valid_robinhood_plus_settings(self) -> RobinhoodPlusConfig:
        ...
        if self.lookback < 2:
            raise ValueError("robinhood_plus.lookback must be >= 2")
        if self.pred_len < 1:
        ...
        for name, value in (("s1_bits", self.s1_bits), ("s2_bits", self.s2_bits)):
