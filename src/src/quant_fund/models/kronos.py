...
class KronosAdapter:
    ...
    def forecast_asset(
        ...
        x_timestamp = pd.DatetimeIndex(history["event_time"])
        y_timestamp = pd.date_range(
            start=x_timestamp[-1] + (x_timestamp[-1] - x_timestamp[-2]),
            periods=self.pred_len,
            freq=x_timestamp[-1] - x_timestamp[-2],
        )
...
def load_local_predictor(
    *, model_path: Path, tokenizer_path: Path, model_sha256: str | None = None, tokenizer_sha256: str | None = None, device: str = "cpu", max_context: int = 512
    ...
    model.eval()
    tokenizer.eval()
    return UpstreamPredictor(model, tokenizer, device=device, max_context=max_context)
