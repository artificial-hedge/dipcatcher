...
from .constants import PRICE_COLS, VOLUME_COL
from .predictor import RobinhoodPlusForecast, RobinhoodPlusPredictor
...
def extract_kline(frame: pl.DataFrame) -> np.ndarray:
    ...
    try:
        arr = frame.select(cols).to_numpy().astype(float, copy=False)
...
class RobinhoodPlusEngine:
    ...
    @classmethod
    def load(cls, path: Path) -> RobinhoodPlusEngine:
...
def forecast_robinhood_plus_cross_section(
    ...
    *,
    ...
    if (
        not kline_columns_present(frame.columns)
        or "event_time" not in frame.columns
        or "security_id" not in frame.columns
    ):
        return {}
    f = frame
    if "available_time" in f.columns:
        f = f.filter(pl.col("available_time") <= pl.lit(asof))
    f = f.filter(pl.col("event_time") <= pl.lit(asof)).sort(["security_id", "event_time"])
    requested = (
        [str(x) for x in security_ids]
        if security_ids is not None
        else [str(x) for x in f["security_id"].unique().to_list()]
    )
    out = {}
