...
def load_pretrained_predictor(
    name="mini", *, model_path=None, tokenizer_path=None, allow_network=False, **kwargs
):
    ...
    if allow_network:
        raise RobinhoodPlusTorchError(
            "network-backed pretrained loading is disabled for research-only backend"
        )
    model = Path(model_path)
    tok = Path(tokenizer_path)
    if not model.is_dir() or not tok.is_dir():
        raise RobinhoodPlusTorchError("local weights are missing; allow_network=false")
    if not torch_available():
        raise RobinhoodPlusTorchError("torch is unavailable; allow_network=false")
...
def forecast_cross_section_torch(frame, asof, config, security_ids):
    ...
    pred = load_pretrained_predictor(
        "mini",
