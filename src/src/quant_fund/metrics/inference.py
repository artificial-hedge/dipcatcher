...
def overlap_aware_hac_lags(n_origins: int, horizon_bars: int) -> int:
    """Return the minimum Bartlett HAC lag for overlapping h-step origins."""
    if isinstance(n_origins, bool) or not isinstance(n_origins, (int, np.integer)):
        raise ValueError("n_origins must be a non-negative integer")
    if int(n_origins) < 0:
    ...
    if horizon <= 0:
        raise ValueError("horizon_bars must be a positive integer")
    return max(0, min(int(n_origins) - 1, horizon - 1))
