...
for tag in ["d1_merged", "h4_merged"]:
    ...
    for asset, d in r["mean_crps_per_asset"].items():
        ...
        best_t = min(tgt.values())
        worst_d = max(dips.values())
        best_d = min(dips.values())
        sweep = worst_d < best_t
        print(
            f"    {asset}: best_target={best_t:.5f} | worst_dip={worst_d:.5f} best_dip={best_d:.5f} | all-dips-beat-all-targets: {sweep}"
        )
