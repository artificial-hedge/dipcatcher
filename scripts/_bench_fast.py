...
def load_workload(sids=SIDS):
    ...
    for i in range(len(times)):
        if i + 1 >= SMA_WINDOW:
            gated = [s for s, c in closes.items()
                     if c[i] > float(np.mean(c[i + 1 - SMA_WINDOW: i + 1]))]
            for sid in closes:
...
def make_cfg():
    ...
    return AppConfig(
        costs=CostConfig(commission_bps=10.0, half_spread_bps=0.0, impact_y=0.0,
                         ...
                         financing_bps_per_year=0.0, frictionless=False,
...
def main():
    ...
    if args.vectorbt:
        import pandas as pd
        frames = {sid: bars.filter(pl.col("security_id") == sid).sort("event_time")
        ...
        def run_vb():
            ...
            return vbt.Portfolio.from_orders(
                close=close_px, size=size, size_type="targetpercent",
                price=open_px, fees=10.0 / 1e4, init_cash=INIT_NAV,
    ...
    out = {"fast_ms_median": float(np.median(ts_fast) * 1e3),
