# Spawn all SOTA eval jobs as WMI-owned processes (survive ssh session teardown).
# Each job: cmd /c wrapper for stdout/stderr redirection.
...
foreach ($seed in @(11, 23)) {
  foreach ($f in @("data\raw\sources\btcusdt_1d_deep.parquet",
    ...
    $jobs += ,@($f, "seed${seed}", $seedargs, $kronos_only)
  }
}
