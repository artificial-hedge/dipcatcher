...
frames = B.load_panel([Path(rf"D:\dipcatcher\data\raw\sources\{s}_1d.parquet") for s in
...
cfg = AppConfig(
    costs=CostConfig(commission_bps=B.COMMISSION_BPS, half_spread_bps=0.0, impact_y=0.0,
        bps_per_turnover=0.0, borrow_bps_per_year=0.0, financing_bps_per_year=0.0,
        frictionless=False, participation_limit=1.0),
    risk_gate=RiskGateConfig(max_order_notional=1e12, max_gross=100.0, max_net=100.0,
        max_name=1.0, max_participation=1.0, max_predicted_vol=100.0,
...
same = all(a == b for a, b in zip(e1, e2)) and len(e1) == len(e2)
print("bitwise identical:", same, "| ref fills:", r_ref.fills.height, "fast fills:", r_fast.fills.height)
if not same:
    for i,(a,b) in enumerate(zip(e1,e2)):
        if a != b:
            print("first divergence at row", i, a, b); break
