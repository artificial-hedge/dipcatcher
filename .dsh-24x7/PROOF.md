...
| latency | **LOSS** — ~1.8× slower: 88 ms vs 50 ms (3-asset), ~5.4×: 446 ms vs 83 ms (11-asset, machine under load — vbt measured 4× its earlier 20 ms). **2026-09-23 update (remote `codex-remote`, receipts `evidence-incumbent-vectorbt-fast{,-11a}-v2.json`):** `run_backtest_fast` (`backtest/fast_replay.py` — numpy-panel replay, bit-identical NAV/fills/metrics asserted by `test_fast_replay.py` incl. 30-seed fuzz) runs the same workloads at **195 ms vs 158 ms (1.23×, 3-asset) / 358 ms vs 159 ms (2.25×, 11-asset)** under light load and **~515–640 ms vs ~235–266 ms (3a) / ~1019–1377 ms vs ~216–290 ms (11a)** under the concurrent SOTA fleet (interleaved reps) — gap narrowed ~43×→~1.2–4.7× depending on host load; vectorbt's numba kernel still faster, the residual is the sequential per-order risk-gate loop in interpreted Python | median of 10 reps; vectorbt is numba-vectorized, dipcatcher is an event loop with per-order risk gates |
| reliability | **WIN** — dipcatcher fails closed on 3/3 injected faults; vectorbt silently accepts all 3 | duplicate weight rows → explicit `ValueError` vs silent acceptance producing a *different* NAV (1,523,565 vs 1,500,764); stale held marks → `StaleValuationError` vs silent fill (equity computed on missing closes without warning); kill-switch halt → 1437 orders blocked, flat equity — vectorbt has no equivalent |
...
The matched workloads produce a **mixed** scorecard, not overperformance: correctness is verified-identical against two independent engines (vectorbt to 2.7e-15; qlib to the float32 quote floor 1e-7 — both certify dipcatcher's accounting, but a tie is not a win), reliability and operability favor dipcatcher, and latency is still a real loss to vectorbt (~1.2–2.3× quiet / ~2.2–4.7× under fleet load on the bit-identical fast path — narrowed ~43×→~1.2×/2.3× from the original engine; the residual is the sequential per-order risk-gate loop vs vectorbt's numba kernel) while being a large win vs qlib (~104×). "Industry-grade" requires overperformance across the quality bar; a head-to-head on UX/security is also unmeasured. Of the three named incumbents: vectorbt executed (exact NAV parity, reliability win, latency loss), Qlib executed (NAV parity at f32 floor, ~104× latency win, silent-halt config trap documented), and Zipline Reloaded attempted and documented not-fair above. The evidence trail is real and rerunnable — the claim is not.
